		...
		if (node.getNodeType() == Node.ELEMENT_NODE) {
			...
			if ("archive".equals(element.getNodeName())) { //$NON-NLS-1$
				...
				if (string == null || string.length() == 0) {
					abort(SourceLookupMessages.getString("ArchiveSourceContainerType.10"), null); //$NON-NLS-1$
				}
				String detect = element.getAttribute("detectRoot"); //$NON-NLS-1$
				boolean auto = "true".equals(detect); //$NON-NLS-1$
				return new ArchiveSourceContainer(string, auto);
			} else {
				abort(SourceLookupMessages.getString("ArchiveSourceContainerType.11"), null); //$NON-NLS-1$
			}
		}
		abort(SourceLookupMessages.getString("ArchiveSourceContainerType.12"), null); //$NON-NLS-1$
		return null;
	}
	/* (non-Javadoc)
