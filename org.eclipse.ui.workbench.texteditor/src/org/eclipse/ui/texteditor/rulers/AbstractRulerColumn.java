 ...
 * <ul>
 * <li>The {@link #setFont(Font) font} used to draw text in
 * {@link #paintLine(GC, int, int, int, int)}.</li>
 ...
 * @since 3.3
 */
public abstract class AbstractRulerColumn extends RulerColumn implements IVerticalRulerInfo, IVerticalRulerInfoExtension {
	...
	private final class InternalListener implements IViewportListener, ITextListener {
		...
		public void textChanged(TextEvent event) {
			...
			if (fWasShowingEntireContents || event.getDocumentEvent() == null || JFaceTextUtil.isShowingEntireContents(getTextWidget()))
				redraw();
		}
	}
	...
	/** The editor's text viewer, possibly <code>null</code>. */
	private ITextViewer fViewer;
	/** The editor's text widget, possibly <code>null</code>. */
	...
	/** The default background color, <code>null</code> to use the editor's background color. */
	 ...
	 */
	public final Control createControl(CompositeRuler parentRuler, Composite parentControl) {
		...
		fViewer= getParentRuler().getTextViewer();
		fViewer.addViewportListener(fInternalListener);
		fViewer.addTextListener(fInternalListener);
...
//				| SWT.NO_MERGE_PAINTS
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Subclasses may extend, but must call super.
	 * </p>
	 */
	protected void columnRemoved() {
		if (fViewer != null) {
			fViewer.removeViewportListener(fInternalListener);
			fViewer.removeTextListener(fInternalListener);
			fViewer= null;
		}
	...
	protected final void redraw(ILineRange lines) {
		...
		int firstWidgetLine= JFaceTextUtil.modelLineToWidgetLine(fViewer, firstModelLine);
		int lastWidgetLine= JFaceTextUtil.modelLineToWidgetLine(fViewer, lastModelLine);
	...
	protected void paint(GC gc, ILineRange lines) {
		...
		for (int line= firstLine; line < lastLine; line++) {
			int modelLine= JFaceTextUtil.widgetLine2ModelLine(fViewer, line);
			if (modelLine == -1)
				continue;
