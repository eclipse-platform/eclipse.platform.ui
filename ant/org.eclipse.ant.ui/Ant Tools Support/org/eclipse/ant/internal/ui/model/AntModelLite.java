...
public class AntModelLite implements IAntModel {

	protected static ClassLoader fgClassLoader;
	protected static int fgInstanceCount= 0;
	
	protected IDocument fDocument;
	protected IProblemRequestor fProblemRequestor;
	protected LocationProvider fLocationProvider;

	protected AntProjectNode fProjectNode;
	private AntTargetNode fCurrentTargetNode;
	protected AntElementNode fLastNode;
     ...
     */
	protected Stack fStillOpenElements = new Stack();
	
	protected Map fTaskToNode= new HashMap();

	private List fTaskNodes= new ArrayList();

	protected final Object fDirtyLock= new Object();
	protected boolean fIsDirty= true;
	private File fEditedFile= null;	
	protected Set fNamesOfOldDefiningNodes;
   
    private ClassLoader fLocalClassLoader= null;
    
    private boolean fHasLexicalInfo= false;
    private boolean fHasPositionInfo= false;
    private boolean fHasTaskInfo= false;
	
	public AntModelLite(IDocument document, IProblemRequestor problemRequestor, LocationProvider locationProvider) {
		fDocument= document;
	...
	protected void cleanup() {
		...
		if (projectNode != null) {
			...
			IntrospectionHelper.getHelper(projectNode.getProject(), AntModelLite.class);
	...
	protected void markHierarchy(AntElementNode openElement, int severity, String message) {
	...
	public void addDTD(String name, int line, int column) {
		//guaranteed to not be called if needsLexicalResolution is false
	    //not support in the lite version
	}
	...
	protected int computeOffset(int line, int column, String prefix) throws BadLocationException {
		int offset;
		if (column <= 0) {
	...
	protected int computeOffsetUsingPrefix(int line, int offset, String prefix, int column) throws BadLocationException {
		String lineText= fDocument.get(fDocument.getLineOffset(line - 1), column);
	...
	protected int getNonWhitespaceOffset(int line, int column) throws BadLocationException {
	...
	protected int getLastCharColumn(int line) throws BadLocationException {
    ...
    /* (non-Javadoc)
     * @see org.eclipse.ant.internal.ui.model.IAntModel#setCanGetLexicalInfo(boolean)
     */
    public void setCanGetLexicalInfo(boolean needsLexicalInfo) {
       fHasLexicalInfo= needsLexicalInfo;
    }

    /* (non-Javadoc)
     * @see org.eclipse.ant.internal.ui.model.IAntModel#setCanGetPositionInfo(boolean)
     */
    public void setCanGetPositionInfo(boolean needsPositionInfo) {
       fHasPositionInfo= needsPositionInfo;   
    }
    
    /* (non-Javadoc)
     * @see org.eclipse.ant.internal.ui.model.IAntModel#setCanGetTaskInfo(boolean)
     */
    public void setCanGetTaskInfo(boolean resolve) {
	    fHasTaskInfo= resolve;
	}
