...
public class FindReplaceDocumentAdapterTest extends TestCase {
	...
	public void testUTF8Pattern() {
		...
		try {
			...
			// Find case-sensitive
			IRegion r= findReplaceDocumentAdapter.find(0, "Gel�st", true, true, false, false); //$NON-NLS-1$
			assertEquals(result, r);
			r= findReplaceDocumentAdapter.find(0, "Gel�st", true, true, false, false); //$NON-NLS-1$
			assertEquals(result, r);
			
			// Find non-case-sensitive
			r= findReplaceDocumentAdapter.find(0, "Gel�st", true, false, false, false); //$NON-NLS-1$
			assertEquals(result, r);
			r= findReplaceDocumentAdapter.find(0, "Gel�st", true, false, false, false); //$NON-NLS-1$
			assertEquals(result, r);
