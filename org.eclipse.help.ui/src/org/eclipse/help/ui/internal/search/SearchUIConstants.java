package org.eclipse.help.ui.internal.search;
/*
 * (c) Copyright IBM Corp. 2000, 2002.
 * All Rights Reserved.
 */
/**
 * Interface for holding UI constants
 */
public interface SearchUIConstants {
	// Search UI pluging id with a "." for convenience.
	public static final String UI_PLUGIN_ID = "org.eclipse.help.ui.";
	public static final String SEARCH_PAGE = UI_PLUGIN_ID + "searchPage";
	public static final String ADVANCED_SEARCH = UI_PLUGIN_ID + "advancedSearch";
	public static final String HIT_MARKER_ID = UI_PLUGIN_ID + "helpsearchhit";
	public static final String HIT_MARKER_ATTR_HREF = "href";
	public static final String HIT_MARKER_ATTR_LABEL = "label";
	public static final String HIT_MARKER_ATTR_RESULTOF = "resultfof";
	public static final String RESULTS_PAGE_ID = UI_PLUGIN_ID + "searchPage";
	public static final String IMAGE_KEY_SEARCH = "search_icon";
	public static final String IMAGE_KEY_TOPIC = "s_topic_icon";
}