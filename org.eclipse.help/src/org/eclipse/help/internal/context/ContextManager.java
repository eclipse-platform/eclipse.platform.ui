...
public class ContextManager {
	...
	private IContextProvider[] contextProviders;
	...
	public IContext getContext(String contextId) {
		...
		IContextProvider[] providers = getContextProviders();
		for (int i=0;i<providers.length;++i) {
			IContext context = null;
			try {
				context = providers[i].getContext(contextId);
				if (context != null) {
					try {
	 ...
	 */
	private IContextProvider[] getContextProviders() {
		if (contextProviders == null) {
			...
			for (int i=0;i<elements.length;++i) {
				...
				try {
					if (elem.getName().equals(ELEMENT_NAME_CONTEXT_PROVIDER)) {
						...
						if (className != null) {
							try {
								IContextProvider provider = (IContextProvider)elem.createExecutableExtension(ATTRIBUTE_NAME_CLASS);
								...
								// log and skip
								String msg = ELEMENT_NAME_CONTEXT_PROVIDER + " class must implement " + IContextProvider.class.getName(); //$NON-NLS-1$
								HelpPlugin.logError(msg, e);
							}
						}
						else {
			...
			contextProviders = (IContextProvider[])providers.toArray(new IContextProvider[providers.size()]);
		}
