...
public abstract class PopupInformationControl implements IInformationControl, IInformationControlExtension {
	...
	public PopupInformationControl(Shell parent, String labelText, String commandId) {
		...
		shell= new Shell(parent, SWT.NO_FOCUS | SWT.ON_TOP | SWT.RESIZE);
		...
		if (command != null) {
			...
			if (keyBindings != null && keyBindings.size() > 0) {
				...
				label.setText(MessageFormat.format(DebugUIMessages.getString("PopupInformationControl.1"), new String[] {lastBinding.getKeySequence().format(), labelText})); //$NON-NLS-1$
