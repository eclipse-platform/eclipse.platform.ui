...
public class ReflectionContributionFactory implements IContributionFactory {
	...
	public Object call(Object object, String uriString, String methodName,
		...
		if (methods.length == 1) {
			...
			Annotation[] EMPTY_ARR = new Annotation[0];
			...
			for (Iterator<Method> iter = toSort.iterator(); iter.hasNext()
				...
				Annotation[][] parameterAnnotations = next
						.getParameterAnnotations();
				for (int i = 0; i < params.length && satisfiable; i++) {
					Class<?> clazz = params[i];
					Annotation[] array = EMPTY_ARR;
					if (parameterAnnotations.length > 0
							&& parameterAnnotations[i].length > 0) {
						array = parameterAnnotations[i];
					}
					if (array != EMPTY_ARR) {
						Named named = (Named) array[0];
						satisfiable = context.containsKey(named.value());
					} else if (!context.containsKey(clazz.getName())
		...
		try {
			logger.log(LogService.LOG_INFO, "calling: " + methodName, null); //$NON-NLS-1$
			...
			for (int i = 0; i < params.length; i++) {
				if (IEclipseContext.class.equals(paramKeys[i])) {
					...
					if (parameterAnnotations.length > i) {
						...
						if (array.length > 0) {
							Named named = (Named) array[0];
							params[i] = context.get(named.value());
						} else {
	...
	public Object create(String uriString, IEclipseContext context) {
		...
		if (bundle != null) {
			...
			logger.log(LogService.LOG_ERROR,
					"Unable to retrive the bundle from the URI: " //$NON-NLS-1$
							+ uriString);
		}
		return contribution;
	}

	private Object createFromBundle(Bundle bundle, IEclipseContext context,
			URI uri) {
		Object contribution;
		if (uri.segmentCount() > 3) {
			String prefix = uri.segment(2);
	...
	private Bundle getBundle(URI platformURI) {
		return TestActivator.getDefault().getBundleForName(
				platformURI.segment(1));
	}
