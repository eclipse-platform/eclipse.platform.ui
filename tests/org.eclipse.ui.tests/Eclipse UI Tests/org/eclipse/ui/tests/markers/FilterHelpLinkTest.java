...
import org.eclipse.ui.tests.FilterNoHelpTestView;
...
public class FilterHelpLinkTest {
	...
	public void helpDoesNotShowWithNoHelpConfig() throws Exception {
		MarkerSupportView view = (MarkerSupportView) PlatformUI.getWorkbench().getActiveWorkbenchWindow()
				.getActivePage().showView(FilterNoHelpTestView.ID);

		Composite composite = showFilterDialog(view);
		assertFalse(filtersDialog_anyResource, isHelpAvailable(composite, JFaceResources.getString("helpToolTip")));
	}

	@Test
	public void helpShowsWithNoHelpConfig() throws Exception {
		MarkerSupportView view = (MarkerSupportView) PlatformUI.getWorkbench().getActiveWorkbenchWindow()
				.getActivePage().showView(FilterHelpTestView.ID);

		Composite composite = showFilterDialog(view);
		assertTrue(filtersDialog_anyResource, isHelpAvailable(composite, JFaceResources.getString("helpToolTip")));
	}
