...
public class ResourceMappingTest extends EclipseWorkspaceTest {
	...
	public void testResourceLocation() {
		...
		IResource[] allResources = new IResource[] {project, mappedFolder, unmappedFolder, folderInMappedFolder, fileInMappedFolder, fileInUnmappedFolder, mappedFile, unmappedFile};
		...
		try {
			...
			ensureExistsInWorkspace(allResources, true);
			for (int i = 0; i < allResources.length; i++) {
				assertEquals("3." + i, projectLocation.append(allResources[i].getProjectRelativePath()), allResources[i].getLocation());
			}
		} catch (CoreException e) {
			fail("3.99", e);
		} finally {
