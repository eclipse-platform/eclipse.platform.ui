...
/**
 * Tests the list binding in the fluent databinding API in the {@link Bind}
 * class.
 */
public class BindListTest extends AbstractDefaultRealmTestCase {
	...
	@Test
	public void setNull() {
		assertThrows(NullPointerException.class, () -> Bind.twoWay().from((IObservableList<Object>) null));
	}

	@Test
	public void oneWayConverter() {
		var target = new WritableList<String>();
		var model = new WritableList<Integer>();
	...
	public void twoWayDefaultConverter() {
		var target = WritableList.withElementType(String.class);
		var model = WritableList.withElementType(Integer.class);
