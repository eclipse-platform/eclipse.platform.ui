...
public FragmentResolveTest_5(String name) {
	super(name);
}
...
public void fullTest() {
	...
	for (int i = 0; i < fragmentIds.length; i++) {
		...
		try {
			...
			String pluginPath = tempPlugin.getLocation().concat("Plugin_Testing/fragment.resolve.5/plugin1.xml");
			pluginURLs[0] = new URL (pluginPath);
			pluginPath = tempPlugin.getLocation().concat("Plugin_Testing/fragment.resolve.5/fragment" + (i + 1) + ".xml");
...
public static Test suite() {
	...
	suite.addTest(new FragmentResolveTest_5("fullTest"));
	return suite;
}
}
