...
/**
 */
public class PluginResolveTest_11 extends PluginResolveTest {
public PluginResolveTest_11() {
	super(null);
}
public PluginResolveTest_11(String name) {
	super(name);
}
public void baseTest() {
	...
	String pluginPath = tempPlugin.getLocation().concat("Plugin_Testing/plugins.resolve.11/");
	...
	verIdC = new PluginVersionIdentifier("2.0.5");
	pluginDescriptor = registry.getPluginDescriptor("tests.c", verIdC);
	assertNotNull("3.0", pluginDescriptor);
	assertEquals("3.1", pluginDescriptor.getUniqueIdentifier(), "tests.c");
	assertEquals("3.2", pluginDescriptor.getVersionIdentifier(), verIdC);
	verIdC = null;
	
}
public static Test suite() {
	TestSuite suite = new TestSuite();
	suite.addTest(new PluginResolveTest_11("baseTest"));
	return suite;
}
}
