...
/**
 */
public class SoftPrereqTest_2 extends PluginResolveTest {
public SoftPrereqTest_2() {
	super(null);
}
public SoftPrereqTest_2(String name) {
	super(name);
}
public void baseTest() {
	...
	String pluginPath = tempPlugin.getLocation().concat("Plugin_Testing/softPrereq.2/");
	...
	assertEquals("2.0", requires.length, 1);
	assertEquals("2.1", requires[0].getUniqueIdentifier(), "tests.b");
	assertTrue("2.2", !requires[0].isMatchedAsPerfect());
	assertTrue("2.3", !requires[0].isMatchedAsEquivalent());
	assertTrue("2.4", !requires[0].isMatchedAsCompatible());
	assertTrue("2.5", !requires[0].isMatchedAsGreaterOrEqual());
	assertTrue("2.6", requires[0].isOptional());
	
	// look for tests.b
	pluginDescriptors = registry.getPluginDescriptors("tests.b");
	assertEquals("3.0", pluginDescriptors.length, 1);
	assertEquals("3.1", pluginDescriptors[0].getUniqueIdentifier(), "tests.b");
	assertEquals("3.2", pluginDescriptors[0].getVersionIdentifier(), new PluginVersionIdentifier("1.1.0"));
}
public static Test suite() {
	TestSuite suite = new TestSuite();
	suite.addTest(new SoftPrereqTest_2("baseTest"));
	return suite;
}
}
