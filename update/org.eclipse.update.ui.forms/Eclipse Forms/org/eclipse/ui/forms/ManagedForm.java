 ...
 * Form toolkit and widget will be created and
...
/**
 * Creates a managed form that will use the
...
/**
 * Initialize all the parts in this form.
 */
...
/**
 * Dispose all the parts in this form.
 */
	public void dispose() {
		for (int i=0; i<parts.size(); i++) {
			IFormPart part = (IFormPart)parts.get(i);
...
/**
 * Refreshes the form by forcing all the parts
 * to reload the content from the model.
 */
	public void refresh() {
		for (int i=0; i<parts.size(); i++) {
			IFormPart part = (IFormPart)parts.get(i);
...
/**
 * Sets the form input. Managed parts could opt
 * to react to it by selecting and/or revealing
 * the object if they contain it.
