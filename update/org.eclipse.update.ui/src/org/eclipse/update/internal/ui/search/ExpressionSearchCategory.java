...
public class ExpressionSearchCategory extends SearchCategory {
	...
	public void createControl(Composite parent, FormWidgetFactory factory) {		
		...
		factory.createLabel(container, UpdateUI.getResourceString(KEY_EXPRESSION));
		expressionText = factory.createText(container, "");
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		expressionText.setLayoutData(gd);
		caseCheck = factory.createButton(container, UpdateUI.getResourceString(KEY_CASE), SWT.CHECK);
		fillHorizontal(caseCheck, 0);
		Label label = factory.createLabel(container, UpdateUI.getResourceString(KEY_LOOK));
		fillHorizontal(label, 0);
		nameCheck = factory.createButton(container, UpdateUI.getResourceString(KEY_NAME), SWT.CHECK);
		fillHorizontal(nameCheck, 10);
		providerCheck = factory.createButton(container, UpdateUI.getResourceString(KEY_PROVIDER), SWT.CHECK);
		fillHorizontal(providerCheck, 10);
		descriptionCheck = factory.createButton(container, UpdateUI.getResourceString(KEY_DESCRIPTION), SWT.CHECK);
