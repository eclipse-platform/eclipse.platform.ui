...
public class WebBrowserView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		if (BootLoader.getWS().equals("win32") == false) {
			Text infoText = new Text(container, SWT.MULTI | SWT.V_SCROLL | SWT.WRAP);
			GridData gd = new GridData(GridData.FILL_BOTH);
			infoText.setLayoutData(gd);
			infoText.setEditable(false);
			infoText.setText(UpdateUIPlugin.getResourceString(KEY_NOT_AVAILABLE));
			return;
		}
	...
	private void createNavBar(Composite parent) {
		...
		makeActions();
		IToolBarManager localBar = getViewSite().getActionBars().getToolBarManager();
	...
	private void makeActions() {
		...
		backwardAction.setToolTipText(UpdateUIPlugin.getResourceString(KEY_BACKWARD));
		backwardAction.setImageDescriptor(UpdateUIPluginImages.DESC_BACKWARD_NAV);
		...
		forwardAction.setToolTipText(UpdateUIPlugin.getResourceString(KEY_FORWARD));
