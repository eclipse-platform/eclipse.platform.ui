...
/**
 * This class is a simple parser implementing the IReadmeFileParser interface.
 * It parses a Readme file into sections based on the existence of numbered
 * section tags in the input. A line beginning with a number followed by a dot
 * will be taken as a section indicator (for example, 1., 2., or 12.). As well,
 * a line beginning with a subsection-style series of numbers will also be taken
 * as a section indicator, and can be used to indicate subsections (for example,
 * 1.1, or 1.1.12).
 */
public class DefaultSectionsParser implements IReadmeFileParser {
	 ...
	 * Each dot in a mark number represents a parent-child separation. For example,
	 * the parent of 1.2 is 1, the parent of 1.4.1 is 1.4. Returns null if there is
	 * no appropriate parent.
	 */
	...
	/**
	 * Returns a string containing the contents of the given file. Returns an empty
	 * string if there were any errors reading the file.
	 */
	...
	/**
	 * Creates a section name from the buffer and trims trailing space characters.
	 *
	...
	/**
	 * Returns the number for this heading. A heading consists of a number (an
	 * arbitrary string of numbers and dots), followed by arbitrary text.
	 */
