...
public class Main {
	...
	protected String searchFor(final String target, String start) {
		...
		ArrayList nameList = new ArrayList(2);
		ArrayList candidateList = new ArrayList(2);
		for (int i = 0; i < candidates.length; i++) {
			File candidate = new File(candidates[i]);
			if (candidate.getName().equals(target) || candidate.getName().startsWith(target + "_")) { //$NON-NLS-1$
				nameList.add(candidate.getName());
		...
		if (result == -1)
			return null;
		File candidate = (File) candidateList.get(result);
