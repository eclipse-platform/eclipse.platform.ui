 ...
 * Design notes:
 * <ul>
 * <li>content providers are only ever created once. The init method is only
 * called once, and so this is why they need to be cached.</li>
 * <li>Content provider ids are used as keys in the hashtable, and their
 * corresponding wrapper classes as values.</li>
 * <li>In the case of HTML presentation, html is cached and so model is not
 * consuloted when we need to redisplay a page. When content provider asks for a
 * reflow, the page is removed from HTML cache and intro model is consulted
 * again. This is when the calls happen to this class.</li>
 * <li>In the case of SWT presentation, same design. SWT pages are cached in a
 * page book. When a content provider needs to refresh a page, the page is
 * removed from the page book and recreated from intro model.</li>
 * </ul>
 */
...
public class ContentProviderManager {
    ...
    class ContentProviderWrapper {
        ...
        ContentProviderWrapper(IIntroContentProvider provider,
                AbstractIntroPage parentPage) {
    ...
    public IIntroContentProvider getContentProvider(
        ...
        ContentProviderWrapper providerWrapper = (ContentProviderWrapper) contentProviders
            .get(provider.getId());
        if (providerWrapper == null)
            // return null if provider has not been created yet.
            return null;
        IIntroContentProvider providerClass = providerWrapper
            .getIIntroContentProvider();
    ...
    public IIntroContentProvider createContentProvider(
        ...
        if (aClass != null && aClass instanceof IIntroContentProvider) {
            ...
            if (provider.getId() != null) {
                ...
                ContentProviderWrapper wrapper = new ContentProviderWrapper(
                    providerClass, provider.getParentPage());
    ...
    public AbstractIntroPage getContentProviderParentPage(
            IIntroContentProvider provider) {
        Enumeration keys = contentProviders.keys();
        while (keys.hasMoreElements()) {
            String key = (String) keys.nextElement();
            ContentProviderWrapper wrapper = (ContentProviderWrapper) contentProviders
                .get(key);
            boolean foundKey = wrapper.getIIntroContentProvider().equals(
                provider) ? true : false;
            if (foundKey) {
    ...
    public void clear() {
        for (Iterator it = contentProviders.values().iterator(); it.hasNext();) {
            ContentProviderWrapper providerWrapper = (ContentProviderWrapper) it
                .next();
            IIntroContentProvider provider = providerWrapper
                .getIIntroContentProvider();
            provider.dispose();
        }
