...
public class PageForm implements IIntroConstants, IPropertyListener {
	...
	// main page book/.
	public static String PAGE_FORM_ID = "pageForm";
	 ...
	 * @param pageBook
	 */
	public void createPartControl(ScrolledPageBook mainPageBook, FormStyleManager styleManager) {
		...
		categoryPageBook = toolkit.createPageBook(formContent.getBody(), SWT.H_SCROLL|SWT.V_SCROLL);
		categoryPageBook.setLayoutData(new GridData(GridData.FILL_BOTH));

		// adding navigation UI.
		Composite navigationComposite = toolkit.createComposite(formContent.getBody());
		navigationComposite.setLayoutData(
		...
		createSmallNavigator(navigationComposite, model.getHomePage().getLinks());
	...
	private Control createImageHyperlink(Composite body, IntroLink link) {
		...
		IPluginDescriptor pd = link.getConfigurationElement().getDeclaringExtension().getDeclaringPluginDescriptor();
		Image image = styleManager.getImage(pd, "rootPage.small-icon."+link.getId(), "rootPage.small-icon", ImageUtil.ROOT_LINK_SMALL);
