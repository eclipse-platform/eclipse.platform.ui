...
public class ContextHelpPart {
	 ...
	 *  
	 */
	public ContextHelpPart(StandbyPart standbyPart) {
		partListener = new PartListener();
		defaultText =
			"Click anywhere in the workbench to see a description of the selected part.";
		ImageUtil.registerImage(ImageUtil.HELP_TOPIC, "help_topic.gif");
		this.standbyPart = standbyPart;
	}
	public Control createPartControl(Composite parent, FormToolkit toolkit) {
		...
		final Button button =
			toolkit.createButton(helpContainer, "Go", SWT.PUSH);
		...
		text.setImage(
			ImageUtil.HELP_TOPIC,
			ImageUtil.getImage(ImageUtil.HELP_TOPIC));
		...
		text.setLayoutData(
			new TableWrapData(TableWrapData.FILL, TableWrapData.FILL));
	...
	private void handlePartActivation(
		...
		if (activated) {
			title.setText(
				"What is \"" + part.getSite().getRegisteredName() + "\"?");
