...
public class CategoryForm implements IIntroConstants, IPropertyListener {
	...
	private HyperlinkAdapter hyperlinkAdapter = new HyperlinkAdapter() {
		public void linkActivated(HyperlinkEvent e) {
			...
			if (parser.hasIntroUrl()) {
				...
				DialogUtil.displayInfoMessage(
					imageLink.getShell(),
					"URL is: " + introLink.getUrl());
		}
	...
	public void createPartControl(
		ScrolledPageBook categoryPageBook,
		...
		boolean hasDivs =
			currentPage.getChildrenOfType(IntroElement.DIV).length != 0
				? true
				: false;

		boolean hasLinks =
			currentPage.getChildrenOfType(IntroElement.LINK).length != 0
				? true
				: false;
	...
	private void createCategoryColumns(
		Composite parent,
	...
	private Control createCategory(
		Composite parent,
		String label,
		String description,
		IntroLink[] links,
		...
		for (int i = 0; i < links.length; i++) {
			Control lc =
				createImageHyperlink(client, links[i], showLinkDescription);
	 ...
	 * @param link
	 */
	private Control createImageHyperlink(
		Composite body,
		IntroLink link,
		...
		AbstractIntroPage currentPage = model.getCurrentPage();
		Image linkImage = styleManager.getImage(currentPage, link, "icon");
		if (showDescription && link.getText() != null) {
			...
			Label desc =
				toolkit.createLabel(container, link.getText(), SWT.WRAP);
			td = new TableWrapData(TableWrapData.FILL, TableWrapData.TOP);
			td.grabHorizontal = true;
			td.grabVertical = true;
			...
			ImageHyperlink imageLink =
				toolkit.createImageHyperlink(body, SWT.WRAP | SWT.CENTER);
			imageLink.setImage(linkImage);
			linkControl = imageLink;
			control = linkControl;
		}
		linkControl.setText(link.getLabel());
		linkControl.setFont(DEFAULT_LINK_LABEL_FONT);
	...
	/**
	 * Handle model property changes. The UI here is notified of a change to
