...
public class BaseExtensionPointManager {
	...
	protected static final String CONFIG_EXTENSION =
	...
	// the attribute in the config element to specify the intro part id.
	protected static final String CONFIG_EXTENSION_CONFIG_ID_ATTRIBUTE =
		"configId";
	...
	protected IntroModelRoot loadModel(
		String attrributeName,
		...
		IConfigurationElement introConfig =
			getIntroConfig(attrributeName, attributeValue);
		...
		if (introConfig != null) {
			...
			if (configId == null)
				...
				introConfigExtensions =
					getIntroConfigExtensions(
						CONFIG_EXTENSION_CONFIG_ID_ATTRIBUTE,
						configId);

			IntroModelRoot model =
				new IntroModelRoot(introConfig, introConfigExtensions);
			model.loadModel();
	 ...
	 * @return
	 */
	protected IConfigurationElement getIntroConfig(
		String attrributeName,
		String attributeValue) {

		IConfigurationElement[] configElements =
			registry.getConfigurationElementsFor(CONFIG);

		IConfigurationElement config =
			getConfigurationFromAttribute(
				configElements,
				attrributeName,
				attributeValue);
		...
		if (config == null)
			...
			Logger.logWarning(
				"No Intro configuration found with "
					+ attrributeName
					+ " of value = "
					+ attributeValue);
	...
	protected IConfigurationElement[] getIntroConfigExtensions(
		...
		IConfigurationElement[] configExtensionElements =
			registry.getConfigurationElementsFor(CONFIG_EXTENSION);

		IConfigurationElement[] configExtensions =
			getConfigurationsFromAttribute(
				configExtensionElements,
				attrributeName,
				attributeValue);
	...
	protected IConfigurationElement getConfigurationFromAttribute(
		...
		IConfigurationElement[] filteredConfigElements =
			getConfigurationsFromAttribute(
				configElements,
				attributeName,
				attributeValue);
	...
	protected IConfigurationElement[] getConfigurationsFromAttribute(
		...
		for (int i = 0; i < configElements.length; i++) {
			String currentAttributeValue =
				configElements[i].getAttribute(attributeName);
		...
		// now return array.
		IConfigurationElement[] filteredConfigElements =
			new IConfigurationElement[elements.size()];
	 ...
	 *         empty.
	 */
	public static IConfigurationElement validateSingleContribution(IConfigurationElement[] configElements) {
