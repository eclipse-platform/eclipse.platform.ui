...
public class IntroPlugin extends AbstractUIPlugin {
	...
	public IntroPlugin(IPluginDescriptor descriptor) {
		...
		inst = this;
		try {
			resourceBundle = ResourceBundle.getBundle("org.eclipse.ui.intro.internal.IntroPluginResources"); //$NON-NLS-1$
		} catch (MissingResourceException x) {
			resourceBundle = null;
			Logger.logWarning("IntroPlugin - unable to load resource bundle"); //$NON-NLS-1$
		}

		// REVISIT: Remove these test messages.
		Logger.logInfo("IntroPlugin class created."); //$NON-NLS-1$
		Logger.logDebugMessage(this.getClass().toString(), "testing debug messages"); //$NON-NLS-1$
	}
	...
	public static String getResourceString(String key) {
		try {
			ResourceBundle bundle =
				IntroPlugin.getDefault().getResourceBundle();
	 ...
	 */
	public static String getFormattedResourceString(
		String key,
		Object[] args) {
