...
public class BrowserIntroPartImplementation
	...
	private BrowserIntroPartLocationListener urlListener =
		new BrowserIntroPartLocationListener(this);

	// Global actions
	private Action backAction = new Action() {
		{
			setToolTipText(
				IntroPlugin.getResourceString("Browser_backward_tooltip")); //$NON-NLS-1$
			setImageDescriptor(
				ImageUtil.createImageDescriptor("backward_nav.gif")); //$NON-NLS-1$
		}
		public void run() {
	...
	private Action forwardAction = new Action() {
		{
			setToolTipText(
				IntroPlugin.getResourceString("Browser_forward_tooltip")); //$NON-NLS-1$
			setImageDescriptor(
				ImageUtil.createImageDescriptor("forward_nav.gif")); //$NON-NLS-1$
		}
		public void run() {
			// dynamic case. Uses navigation history.
			if (getModelRoot().getHomePage().isDynamic()) {
	...
	private Action homeAction = new Action() {
		{
			setToolTipText(
				IntroPlugin.getResourceString("Browser_home_tooltip")); //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		if (!getModelRoot().hasValidConfig()) {
			browser.setText(
				IntroPlugin.getResourceString("BrowserIntroPartImplementation.invalidConfig")); //$NON-NLS-1$
			return;
		}
	...
	private void generateDynamicContentForPage(AbstractIntroPage page) {
		...
		HTMLElement html =
			getHTMLGenerator().generateHTMLforPage(
				page,
