...
public class TextIntroPartImplementation extends AbstractIntroPart {
	...
	public void createPartControl(Composite container) {
		Text textUI =
			new Text(container, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL);
	...
	private void printModelRootInfo(IntroModelRoot model, StringBuffer text) {
		...
		text.append(
			"\nPresentation Title = " + model.getPresentation().getTitle());
		text.append(
		...
		text.append(
			"\nNumber of unresolved extensions = "
				+ model.getChildrenOfType(
					IntroElement.CONTAINER_EXTENSION).length);
	}
	...
	private void printContainerChildren(
	...
	private void printModelFlagTests(IntroModelRoot model, StringBuffer text) {
		...
		text.append(
			"\n\t\t\tModel Elements: "
				+ firstPage.getChildrenOfType(IntroElement.BASE_ELEMENT).length);
		text.append(
			"\n\t\t\tContainers: "
				+ firstPage.getChildrenOfType(
					IntroElement.ABSTRACT_CONTAINER).length);
		text.append(
			"\n\t\t\tAll Pages: "
				+ firstPage.getChildrenOfType(IntroElement.ABSTRACT_PAGE).length);
		text.append(
			"\n\t\t\tElements with Text child(AbstractTextElemets): "
				+ firstPage.getChildrenOfType(IntroElement.ABSTRACT_TEXT).length);

		IntroElement[] linksAndDivs =
			(IntroElement[]) firstPage.getChildrenOfType(
				IntroElement.DIV | IntroElement.LINK);
		text.append("\n\t\t\tDivs and Links: " + linksAndDivs.length);
		//		text.append("\n\t\t\tURLtesting: ");
		//		try {
		//			URL url =
		//				IntroPlugin.getDefault().find(new Path(firstPage.getTitle()));
		...
		//				new BufferedReader(
		//					new InputStreamReader(stream, Platform.getNL()));
		//			//reader.
		//			int ch;
