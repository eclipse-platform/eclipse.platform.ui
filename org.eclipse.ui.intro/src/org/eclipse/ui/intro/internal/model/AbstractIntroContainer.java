...
public abstract class AbstractIntroContainer extends IntroElement {
	...
	public IntroElement[] getChildren() {
		...
		IntroElement[] childrenElements =
			(IntroElement[]) convertToModelArray(children,
				IntroElement.BASE_ELEMENT);
		return childrenElements;
	}

	/**
	 * Returns all the children of this container that are of the specified
	 * type(s). <br>An example of an element mask is as follows:
	 * <p>
	 * <code>
	...
	protected void addChildren(IConfigurationElement[] childElements) {
		...
		for (int i = 0; i < childElements.length; i++) {
			...
			if (childElements[i].getName().equals(IntroDiv.DIV_ELEMENT))
				child = new IntroDiv(childElements[i]);
			else if (
				childElements[i].getName().equalsIgnoreCase(
					IntroLink.LINK_ELEMENT))
				child = new IntroLink(childElements[i]);
			else if (
				childElements[i].getName().equalsIgnoreCase(
					IntroText.TEXT_ELEMENT))
				child = new IntroText(childElements[i]);
			else if (
				childElements[i].getName().equalsIgnoreCase(
					IntroImage.IMAGE_ELEMENT))
				child = new IntroImage(childElements[i]);
			else if (
				childElements[i].getName().equalsIgnoreCase(
					IntroHTML.HTML_ELEMENT))
				child = new IntroHTML(childElements[i]);
			else if (
				childElements[i].getName().equalsIgnoreCase(
					IntroInclude.INCLUDE_ELEMENT))
				child = new IntroInclude(childElements[i]);
	...
	private IntroElement findIncludeTarget(IntroInclude include) {
		...
		IntroModelRoot targetModelRoot =
			(IntroModelRoot) getParentPage().getParent();
		String targetConfigID = include.getConfigId();
		if (targetConfigID != null)
			targetModelRoot =
				ExtensionPointManager.getInst().getModel(targetConfigID);
	 ...
	 * @param target
	 */
	private void handleIncludeStyleInheritence(
		IntroInclude include,
		IntroElement target) {
