...
public class OpenResourceAction extends WorkspaceAction implements IResourceChangeListener {
	...
	public void run() {
		...
		List<IProject> nestedProjects = promptForNestedProjects ? NestedProjects.below(projects, false)
	...
	/**
	 * Sets whether {@link #run()} also handles closed projects nested below the
	 * selected ones, as the workspace preference says or by asking. Turned off,
	 * the action opens exactly the selected projects, with their references as
	 * before.
	 *
	...
	private static boolean hasClosedReferences(List<? extends IResource> projects, List<IProject> nestedProjects) {
		Set<IResource> candidates = new LinkedHashSet<>(projects);
