 ...
 * The owner of this object is responsible for adding and removing the items in
 * the table (by invoking the add, remove, set, and change methods,
 * respectively). However, the changes are buffered and applied to the actual
 * table incrementally using a background thread. This keeps the UI responsive
 * when manipulating very large tables.
 * 
 * <p>
 * Other objects should set the contents of the table by manipulating this
 * object rather than manipulating the table itself. Threading issues are
...
class MarkerContentProvider implements ITreeContentProvider {
	...
	// NLS strings
	private static final String TABLE_SYNCHRONIZATION = MarkerMessages.TableContentProvider_TableSynchronization;

	private static final String UPDATING_TABLE_WIDGET = MarkerMessages.TableContentProvider_Updating;
	 ...
	 * viewer. It will either add, remove, or change items in the viewer
	 * depending on the contents of the pending* sets, above. It is scheduled
	 * repeatedly by the OverallUpdateJob, below.
	 */
	private class WidgetRefreshJob extends WorkbenchJob {
		...
		/**
		 * Will be executed each time the update thread wakes up. This makes a
		 * single incremental update to the viewer (ie: adds or removes a few
		 * items)
		 */
		public IStatus runInUIThread(IProgressMonitor monitor) {

			// If we can't get the lock, terminate without blocking the UI
			// thread.
			if (lock.getDepth() > 0) {
				lastWorked = 0;
				return Status.OK_STATUS;
	 ...
	 * been applied. It repeatedly schedules WidgetRefreshJobs until there are
	 * no more changes to apply. This job doesn't actually do any real work --
	 * it simply schedules updates and updates the progress bar.
	 */
	...
	/**
	 * Creates a new TableContentProvider that will control the contents of the
	 * given viewer.
	 * 
	 * @param view
	 * @param description
	 *            user-readable string that will be included in progress
	 *            monitors
	 * @param service
	 *            IWorkbenchSiteProgressService or <code>null</null>
	 ...
	 * 
	 * @param c
	 *            comparator that controls the view's sort order (or null if no
	 *            sorting)
	 */
	...
	/**
	 * Sets the contents of the table. Note that the changes may not become
	 * visible immediately, as the viewer will actually be updated in a
	 * background thread.
	 * 
	...
	/**
	 * If the contents of the viewer have somehow become out-of-sync with the
	 * "visibleItems" set, this method will restore synchronization.
	 */
	private void resync() {
		if (controlExists()) {
	 ...
	 * 
	 * @param changes
	 *            collection of objects that have changed
	 */
	...
	/**
	 * Returns an estimate of the total work remaining (used for progress
	 * monitors)
	 * 
	 * @return
	 */
	...
	/**
	 * Starts the update thread... this will continue to make incremental
	 * changes to the viewer until the pending* sets become empty. Does nothing
	 * if the update thread is already running or if there are no changes to
	 * process.
	 */
	 ...
	 * method before performing a long computation that will ultimately
	 * invalidate the contents of the viewer.
	 */
	...
	private void doUpdate(IProgressMonitor monitor) throws InterruptedException {
		...
		// This counter represents how many work units remain unused in the
		// progress monitor.
		...
		try {
			...
			// Loop until there are no more changes to apply, the control is
			// destroyed, the monitor is cancelled,
			...
			while (hasPendingChanges() && !monitor.isCanceled()) {
				...
				try {
					...
					try {
						uiJob.schedule();
						// Wait for the current update job to complete before
						// scheduling another
						uiJob.join();
					} catch (IllegalStateException e) {
						// Ignore this exception -- it means that the Job
						// manager was shut down, which is expected
						// at the end of the application. Note that we need to
						// check for this by catching the exception
						// rather than using something like if
						// (jobManagerIsShutDown())... since the job manager
						// might
						// be shut down in another thread between the time we
						// evaluate the if statement and when
						// we schedule the job.
					}

					// Estimate how much of the remaining work we'll be doing in
					// this update,
	...
	/**
	 * Performs a single update to the viewer. Based on the contents of the
	 * pending* queues, items will either be removed, added, or refreshed in the
	 * viewer (in that order). This should only be called within a synchronized
	 * block, since the various queues shouldn't be modified during an update.
	 * This method is invoked repeatedly by jobs to gradually apply the pending
	 * changes.
	 */
	private int updateViewer() {
		...
		// Note that this method is only called when we have the lock so
		// acquiring it here
		// does nothing... but we re-acquire it anyway in case future
		// refactoring causes
		...
		if (element instanceof ConcreteMarker) {
			...
			for (int i = 0; i < elements.length; i++) {
				if (elements[i].equals(element))
					return queues.getView().getViewer().getInput();
			}
		}
		return null;
	}
	...
	public boolean hasChildren(Object element) {
		return queues.getView().getViewer().getInput().equals(element);
