...
public class WizardZipFileImportPage1 extends WizardFileSystemImportPage1
     ...
     */
    public WizardZipFileImportPage1(IWorkbench aWorkbench,
    ...
    protected void displaySelectedCount(int selectedEntryCount) {
        if (selectedEntryCount == 1)
            detailsDescriptionLabel.setText(DataTransferMessages
                    .getString("ImportPage.oneSelected")); //$NON-NLS-1$
        else
            detailsDescriptionLabel
                    .setText(DataTransferMessages
                            .format(
                                    "ZipImport.entriesSelected", new Object[] { String.valueOf(selectedEntryCount) })); //$NON-NLS-1$
    }
    ...
    protected boolean importAllResources() {
        ...
        return executeImportOperation(new ImportOperation(
                getContainerFullPath(), structureProvider.getRoot(),
                structureProvider, this));
    }
        ...
        if (settings != null) {
            ...
            if (importTypedResourcesRadio.getSelection())
                selectedTypesNames = addToHistory(selectedTypesNames,
                        typesToImportField.getText());
            settings.put(STORE_SELECTED_TYPES_ID, selectedTypesNames);

            // radio buttons and checkboxes	
            settings.put(STORE_IMPORT_ALL_RESOURCES_ID, importAllResourcesRadio
                    .getSelection());
