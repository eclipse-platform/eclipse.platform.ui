...
public class VModelComponentUIEditor extends AbstractComponentEditor {
	private Composite composite;
	private EMFDataBindingContext context;
	private ModelEditor editor;
	private TableViewer viewer;

	public VModelComponentUIEditor(EditingDomain editingDomain, ModelEditor editor) {
		super(editingDomain);
		this.editor = editor;
	}
	...
	public String getDescription(Object element) {
		return "UI-Children Bla Bla Bla Bla Bla";
	}
	...
	private Composite createForm(Composite parent, EMFDataBindingContext context,
...

		{
				...
				public void widgetSelected(SelectionEvent e) {
					if( ! viewer.getSelection().isEmpty() ) {
						...
						if( s.size() == 1 ) {
							...
							MModelComponent container = (MModelComponent) getMaster().getValue();
							int idx = container.getDescriptors().indexOf(obj) - 1;
							if( idx >= 0 ) {
								Command cmd = MoveCommand.create(getEditingDomain(), getMaster().getValue(), ApplicationPackageImpl.Literals.MODEL_COMPONENT__CHILDREN, obj, idx);
			...
			childrenDropDown.setInput(new EClass[] { 
					...
					BasicPackageImpl.Literals.TRIM_ELEMENT,
			...
			b.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					MUIElement obj = (MUIElement) EcoreUtil.create(eClass);
					
					Command cmd = AddCommand.create(getEditingDomain(), getMaster().getValue(), ApplicationPackageImpl.Literals.MODEL_COMPONENT__CHILDREN, obj);
			...
			b.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					if( ! viewer.getSelection().isEmpty() ) {
						...
						Command cmd = RemoveCommand.create(getEditingDomain(), getMaster().getValue(), ApplicationPackageImpl.Literals.MODEL_COMPONENT__CHILDREN, commands);
						if( cmd.canExecute() ) {
							getEditingDomain().getCommandStack().execute(cmd);
						}
					}
