...
/**
 * A Web browser widget. It extends the Eclipse SWT Browser widget by adding
 * an optional toolbar complete with a URL combo box, history, back & forward,
 * and refresh buttons.
 * <p>
...
public class BrowserViewer extends Composite {
	/**
	 * Style parameter (value 1) indicating that the URL and Go button
	 * will be on the local toolbar.
	 */
	public static final int LOCATION_BAR = 1 << 1;

	/**
	 * Style parameter (value 2) indicating that the toolbar
	 * will be available on the web browser. This style parameter
	 * cannot be used without the LOCATION_BAR style.
	 */
	...
	/**
	 * Creates a new Web browser given its parent and a style value
	 * describing its behavior and appearance.
	 * <p>
	 * The style value is either one of the style constants defined in the
	 * class header or class <code>SWT</code> which is applicable to
	 * instances of this class, or must be built by <em>bitwise OR</em>'ing
	 * together (that is, using the <code>int</code> "|" operator) two or
	 * more of those <code>SWT</code> style constants. The class description
	...
	private void addBrowserListeners() {	
		// respond to ExternalBrowserInstance StatusTextEvents events by updating the status line
		...
		browser.addProgressListener(new ProgressListener() {
			public void changed(ProgressEvent event) {
				...
				if (showToolbar) {
					if (!busy.isBusy() && !done)
						loading = true;
					else if (busy.isBusy() && done) // once the progress hits 100 percent, done, set busy to false
						loading = false;
	 ...
	 * Convenience method that calls the underlying SWT browser.
	 *
