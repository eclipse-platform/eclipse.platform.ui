...
public class IDEWorkbenchWindowAdvisor extends WorkbenchWindowAdvisor {
	...
	static boolean promptOnExit(Shell parentShell) {
		...
		if (promptOnExit) {
			...
			MessageDialogWithToggle dlg = MessageDialogWithToggle.openOkCancelConfirm(parentShell, IDEWorkbenchMessages.PromptOnExitDialog_shellTitle, message,
					...
					if (pi != null) {
						...
						if (bundle != null) {
							try {
								...
								StatusManager.getManager().handle(new Status(IStatus.ERROR, IDEApplication.PLUGIN_ID, "Failed to load feature", exception));//$NON-NLS-1$
							}
						}
					}
				}
