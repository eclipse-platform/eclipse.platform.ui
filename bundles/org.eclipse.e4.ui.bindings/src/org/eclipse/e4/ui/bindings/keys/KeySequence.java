...
/**
 * <p>
 * A <code>KeySequence</code> is defined as a list of zero or more <code>KeyStrokes</code>, with the
 * stipulation that all <code>KeyStroke</code> objects must be complete, save for the last one,
 * whose completeness is optional. A <code>KeySequence</code> is said to be complete if all of its
 * <code>KeyStroke</code> objects are complete.
 * </p>
 * <p>
 * All <code>KeySequence</code> objects have a formal string representation available via the
 * <code>toString()</code> method. There are a number of methods to get instances of
 * <code>KeySequence</code> objects, including one which can parse this formal string
 * representation.
 * </p>
 * <p>
 * All <code>KeySequence</code> objects, via the <code>format()</code> method, provide a version of
 * their formal string representation translated by platform and locale, suitable for display to a
 * user.
 * </p>
 * <p>
 * <code>KeySequence</code> objects are immutable. Clients are not permitted to extend this class.
 * </p>
 * 
 * @since 3.1
 */
public final class KeySequence extends TriggerSequence implements Comparable {
	 ...
	 */
	private final static KeySequence EMPTY_KEY_SEQUENCE = new KeySequence(new KeyStroke[0]);

	/**
	 * The delimiter between multiple key strokes in a single key sequence -- expressed in the
	 * formal key stroke grammar. This is not to be displayed to the user. It is only intended as an
	 * internal representation.
	 */
	public final static String KEY_STROKE_DELIMITER = "\u0020"; //$NON-NLS-1$

	/**
	 * The set of delimiters for <code>KeyStroke</code> objects allowed during parsing of the formal
	 ...
	 * 
	 * @return a key sequence. This key sequence will have no key strokes. Guaranteed not to be
	 *         <code>null</code>.
	 */
	...
	/**
	 * Creates an instance of <code>KeySequence</code> given a key sequence and a key stroke.
	 * 
	 * @param keySequence
	 *            a key sequence. Must not be <code>null</code>.
	 * @param keyStroke
	 *            a key stroke. Must not be <code>null</code>.
	 * @return a key sequence that is equal to the given key sequence with the given key stroke
	 *         appended to the end. Guaranteed not to be <code>null</code>.
	 */
	 ...
	 * 
	 * @param keyStrokes
	 *            the array of key strokes. This array may be empty, but it must not be
	 *            <code>null</code>. This array must not contain <code>null</code> elements.
	 * @return a key sequence. Guaranteed not to be <code>null</code>.
	 */
	 ...
	 * 
	 * @param keyStrokes
	 *            the list of key strokes. This list may be empty, but it must not be
	 *            <code>null</code>. If this list is not empty, it must only contain instances of
	 *            <code>KeyStroke</code>.
	 * @return a key sequence. Guaranteed not to be <code>null</code>.
	 */
	public static final KeySequence getInstance(final List keyStrokes) {
		return new KeySequence((KeyStroke[]) keyStrokes.toArray(new KeyStroke[keyStrokes.size()]));
	}

	/**
	 * Creates an instance of <code>KeySequence</code> by parsing a given formal string
	 * representation.
	 * 
	 * @param string
	 ...
	 * 
	 * @return A string representation for this key sequence using the default look; never
	 *         <code>null</code>.
	 */
	 ...
	 * 
	 * @return the list of key strokes keys. This list may be empty, but is guaranteed not to be
	 *         <code>null</code>. If this list is not empty, it is guaranteed to only contain
	 *         instances of <code>KeyStroke</code>.
	 */
	...
	/**
	 * Returns whether or not this key sequence is complete. Key sequences are complete iff all of
	 * their key strokes are complete.
	 * 
	 ...
	 * 
	 * @return The formal string representation for this key sequence. Guaranteed not to be
	 *         <code>null</code>.
