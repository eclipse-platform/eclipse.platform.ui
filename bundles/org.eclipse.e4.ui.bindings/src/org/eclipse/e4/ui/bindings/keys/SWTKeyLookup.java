...
/**
 * <p>
 * A look-up table for the formal grammar for keys, and the integer values they represent. This
 * look-up table is hard-coded to use SWT representations. By replacing this class (and
 * {@link org.eclipse.e4.ui.bindings.keys.SWTKeySupport}), you can remove the dependency on SWT.
 * </p>
 * 
 * @since 3.1
 * @see org.eclipse.e4.ui.bindings.keys.KeyLookupFactory
 */
public final class SWTKeyLookup implements IKeyLookup {

	/**
	 * The look-up table for modifier keys. This is a map of formal name (<code>String</code>) to
	 * integer value (<code>Integer</code>).
	 */
	private final Map modifierKeyTable = new HashMap();

	/**
	 * The look-up table for formal names. This is a map of integer value (<code>Integer</code>) to
	 * formal name (<code>String</code>).
	 */
	private final Map nameTable = new HashMap();

	/**
	 * The look-up table for natural keys. This is a map of formal name (<code>String</code>) to
	 * integer value (<code>Integer</code>).
	 */
	private final Map naturalKeyTable = new HashMap();

	/**
	 * Constructs a new look-up class. This should only be done by the look-up factory.
	 * 
	...
	SWTKeyLookup() {
		...
		modifierKeyTable.put(M1_NAME, Util.isMac() ? command : ctrl);
		modifierKeyTable.put(M2_NAME, shift);
		modifierKeyTable.put(M3_NAME, alt);
		modifierKeyTable.put(M4_NAME, Util.isMac() ? ctrl : command);
