...
/**
 * <p>
 * A sequence of one or more triggers. None of these triggers may be <code>null</code>.
 * </p>
 * 
 * @since 3.1
 */
public abstract class TriggerSequence {
	 ...
	 * of the values on which it is based changes. This values is
	 * <code>HASH_CODE_NOT_COMPUTED</code> iff the hash code has not yet been computed.
	 */
	protected transient int hashCode = HASH_CODE_NOT_COMPUTED;

	/**
	 * The list of trigger in this sequence. This value is never <code>null</code>, and never
	 ...
	 * 
	 * @param triggers
	 *            The triggers contained within this sequence; must not be <code>null</code> or
	 *            contain <code>null</code> elements. May be empty.
	 */
	 ...
	 * @param equals
	 *            whether or not an identical trigger sequence should be considered as a possible
	 *            match.
	 * @return <code>true</code>, iff the given trigger sequence ends with this trigger sequence.
	 */
	public final boolean endsWith(final TriggerSequence triggerSequence, final boolean equals) {
		if (triggerSequence == null) {
			throw new NullPointerException("Cannot end with a null trigger sequence"); //$NON-NLS-1$
		}
	 ...
	 * 
	 * @return A string representation for this trigger sequence using the default look; never
	 *         <code>null</code>.
	 */
	 ...
	 * <code>TriggerSequence</code>. A prefix is also an instance of <code>TriggerSequence</code>.
	 * </p>
	 * <p>
	 * For example, consider a trigger sequence that consists of four triggers: A, B, C and D. The
	 * prefixes would be "", "A", "A B", and "A B C". The list of prefixes must always be the same
	 * as the size of the trigger list.
	 * </p>
	 * 
	 * @return The array of possible prefixes for this sequence. This array must not be
	 *         <code>null</code>, but may be empty. It must only contains instances of
	 ...
	 * 
	 * @return The triggers; never <code>null</code> and guaranteed to only contain instances of
	 *         <code>Trigger</code>.
	 */
	 ...
	 *            whether or not an identical trigger sequence should be considered as a possible
	 *            match.
	 * @return <code>true</code>, iff the given trigger sequence starts with this key sequence.
	 */
	public final boolean startsWith(final TriggerSequence triggerSequence, final boolean equals) {
		if (triggerSequence == null) {
			throw new NullPointerException("A trigger sequence cannot start with null"); //$NON-NLS-1$
		}
