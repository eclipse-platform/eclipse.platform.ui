...
public final class RegistryChangeEvent implements IRegistryChangeEvent {
	...
	private HostDelta[] getHostDeltas() {
		// if there is a filter, return only the delta for the selected plug-in
		if (filter != null) {
			HostDelta singleDelta = getHostDelta(filter);
			return singleDelta == null ? new HostDelta[0] : new HostDelta[] { singleDelta };
		}
		// there is no filter - return all deltas
		return (HostDelta[]) deltas.values().toArray(new HostDelta[deltas.size()]);
	}
	private HostDelta getHostDelta(String pluginId) {
		if (filter != null && !pluginId.equals(filter))
			return null;
		return (HostDelta) deltas.get(pluginId);
	}
	public IExtensionDelta[] getExtensionDeltas() {
		HostDelta[] hostDeltas = getHostDeltas();
	...
	public IExtensionDelta[] getExtensionDeltas(String hostName) {
		HostDelta hostDelta = getHostDelta(hostName);
		if (hostDelta == null)
			return new IExtensionDelta[0];
