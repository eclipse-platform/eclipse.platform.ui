	...
	public void run(IProgressMonitor pm) throws CoreException {
		...
		try {
			...
			if (fCheckConditionOperation != null) {
				...
				fCheckConditionOperation.run(SubMonitor.convert(pm, conditionTicks));
				...
				if (status != null && status.getSeverity() < fConditionCheckingFailedSeverity) {
					fChange= fRefactoring.createChange(SubMonitor.convert(pm, rtp.getCreateChangeTicks()));
					fChange.initializeValidationData(new NotCancelableProgressMonitor(
							SubMonitor.convert(pm, rtp.getInitializeChangeTicks())));
				} else {
