 ...
 *     if (!change.isEnabled())
 *         return;
 *     RefactoringStatus valid= change.isValid(SubMonitor.convert(pm, 1));
 *     if (valid.hasFatalError())
 *         return;
 *     Change undo= change.perform(SubMonitor.convert(pm, 1));
 *     if (undo != null) {
 *        undo.initializeValidationData(SubMonitor.convert(pm, 1));
