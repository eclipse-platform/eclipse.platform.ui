...
import org.eclipse.jface.progress.NotifyingJob;
...
public class WizardNewFileCreationPage extends WizardPage implements Listener {
	...
	protected void createAdvancedControls(Composite parent) {
		...
		linkedResourceGroup =
			new CreateLinkedResourceGroup(IResource.FILE, new Listener() {
	...
	protected void handleAdvancedButtonSelect() {
		...
		if (linkedResourceGroupHeight == -1) {
			Composite linkedResourceComposite =
				linkedResourceGroup.createContents(composite);
			Point groupSize =
				linkedResourceComposite.computeSize(
					SWT.DEFAULT,
					SWT.DEFAULT,
					true);
		...
		if (initialContainerFullPath != null)
			...
			if (enum.hasNext()) {
				...
				if (object instanceof IResource) {
					...
					selectedResource =
						(IResource) ((IAdaptable) object).getAdapter(
							IResource.class);
				}
				if (selectedResource != null) {
					...
					if (selectedResource.isAccessible())
						resourceGroup.setContainerFullPath(
							selectedResource.getFullPath());
				}
			}
		}
	}
	...
	protected IStatus validateLinkedResource() {
		...
		IStatus status =
			linkedResourceGroup.validateLinkLocation(newFileHandle);
	...
	protected boolean validatePage() {
		...
		if (!resourceGroup.areAllValuesValid()) {
			...
			if (resourceGroup.getProblemType()
				== ResourceAndContainerGroup.PROBLEM_RESOURCE_EMPTY
	 ...
	 *
	 *<p>
	 * The file creation is done in the background and the result (an IFile)
	 * will be passed on to the listener.
	 */
	public void createNewFileDeferred(final ResourceCompletionListener listener) {
		if (newFile != null)
			return;
			...
			public IStatus run(IProgressMonitor monitor) {
				...
				IWorkspaceRunnable workspaceRunnable =
					...
					public void run(IProgressMonitor pm) throws CoreException {
						pm.beginTask(WorkbenchMessages.getString("WizardNewFileCreationPage.progress"), 2000); //$NON-NLS-1$
						ContainerGenerator generator =
							new ContainerGenerator(containerPath);
						generator.generateContainer(
