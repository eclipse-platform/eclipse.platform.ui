...
public class ConfigurationLogDefaultSection implements ISystemSummarySection {
	...
	private void appendProperties(PrintWriter writer) {
		writer.println();
		writer.println(WorkbenchMessages.getString("SystemSummary.systemProperties")); //$NON-NLS-1$
	...
	private void appendFeatures(PrintWriter writer) {
        ...
        for(int i = 0; i < bundleGroupInfos.length; ++i) {
        	...
        	writer.println(WorkbenchMessages.format("SystemSummary.featureVersion", args)); //$NON-NLS-1$
        }
	}	
	...
	private void appendRegistry(PrintWriter writer) {
		writer.println();
		writer.println(WorkbenchMessages.getString("SystemSummary.pluginRegistry")); //$NON-NLS-1$
        ...
        for(int i = 0; i < bundleInfos.length; ++i) {
        	...
        	String[] args = new String[] { info.getId(), info.getVersion(), info.getName()};
        	writer.println(WorkbenchMessages.format("SystemSummary.descriptorIdVersion", args)); //$NON-NLS-1$
        }
	}	
	...
	private void appendUserPreferences(PrintWriter writer) {
		...
		writer.println();
		writer.println(WorkbenchMessages.getString("SystemSummary.userPreferences")); //$NON-NLS-1$
