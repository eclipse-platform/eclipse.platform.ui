...
public class DecoratorRegistryReader extends RegistryReader {
    ...
    public boolean readElement(IConfigurationElement element) {
        ...
        IConfigurationElement[] enablement = element
        ...
        if (enablement.length == 0) {
            ...
            if (className == null) {
                ...
                enablementExpression = new ActionExpression(ATT_OBJECT_CLASS,
                        className);
        } else
            enablementExpression = new ActionExpression(enablement[0]);

        boolean noClass = element.getAttribute(ATT_CLASS) == null;
        ...
        if (P_TRUE.equals(element.getAttribute(ATT_LIGHTWEIGHT)) || noClass) {
            ...
            int quadrant = getLocationConstant(element
                    .getAttribute(ATT_LOCATION), element);
            ...
            values.add(new LightweightDecoratorDefinition(id, name,
                    description, enablementExpression, adaptable, enabled,
                    quadrant, iconPath, element));
        } else {
            values.add(new FullDecoratorDefinition(id, name, description,
                    enablementExpression, adaptable, enabled, element));
        }
