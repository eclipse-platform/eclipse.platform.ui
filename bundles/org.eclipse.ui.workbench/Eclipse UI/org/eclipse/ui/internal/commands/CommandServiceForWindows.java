...
final class CommandServiceForWindows {
	...
	private CommandServiceForParts activeCommandServiceForParts;
	private SortedMap globalActionsCommandIdToActionMap = new TreeMap();
	private WorkbenchWindow workbenchWindow;
	
	CommandServiceForWindows(WorkbenchWindow workbenchWindow) {
	...
	IAction getAction(String command) {
		...
		if (activeCommandServiceForParts != null)
			action = (IAction) activeCommandServiceForParts.getAction(command);
	...
	void pressed(Stroke stroke, Event event) { 
		...
		if (childSequenceMapForMode.isEmpty()) {
			...
			if (command != null && activeCommandServiceForParts != null) {
	...
	void update(IWorkbenchPart workbenchPart) {
		String[] activeContexts = new String[] { "" }; //$NON-NLS-1$

		if (workbenchPart != null) {
			// TODO:
			//activeCommandServiceForParts = (CommandServicePart) workbenchPart.getSite().getKeyBindingService();
		
			if (activeCommandServiceForParts != null)
				activeContexts = activeCommandServiceForParts.getActiveContexts();
		}

		try {
			Manager.getInstance().getKeyMachine().setScopes(activeContexts);
