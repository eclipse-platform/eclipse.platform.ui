...
final class Persistence {
    ...
    final static String TAG_HANDLER = "handler"; //$NON-NLS-1$
    ...
    private static int parseDeprecatedStroke(String string) {
        ...
        if (size % 2 == 1) {
            ...
            Integer integer = (Integer) stringToValueMap.get(token
                    .toUpperCase());
                ...
                for (int i = 0; i < size - 1; i++) {
                    ...
                    if (i % 2 == 0) {
                        if (token.equalsIgnoreCase(CTRL)) {
                            ...
                            if ((value & SWT.ALT) != 0) return 0;

                            value |= SWT.ALT;
                        } else if (token.equalsIgnoreCase(SHIFT)) {
                            if ((value & SWT.SHIFT) != 0) return 0;
                            ...
                            return 0;
                    } else if (!MODIFIER_SEPARATOR.equals(token)) return 0;
                }
            }
        }
    ...
    static ActiveKeyConfigurationDefinition readActiveKeyConfigurationDefinition(
            IMemento memento, String pluginIdOverride) {
        ...
        String pluginId = pluginIdOverride != null ? pluginIdOverride : memento
                .getString(TAG_PLUGIN_ID);
    ...
    static List readActiveKeyConfigurationDefinitions(IMemento memento,
            String name, String pluginIdOverride) {
    ...
    static List readContextBindingDefinitions(IMemento memento, String name,
        ...
        for (int i = 0; i < mementos.length; i++)
            list
                    .add(readContextBindingDefinition(mementos[i],
                            pluginIdOverride));
    ...
    static CommandDefinition readCommandDefinition(IMemento memento,
        ...
        // TODO deprecated start
        if (categoryId == null) categoryId = memento.getString("category"); //$NON-NLS-1$ 
        // TODO deprecated end
     ...
     *         <code>null</code>.
     */
    static IHandler readHandlerDefinition(
    ...
    static ImageBindingDefinition readImageBindingDefinition(IMemento memento,
            String pluginIdOverride) {
        if (memento == null) throw new NullPointerException();
        ...
        String pluginId = pluginIdOverride != null ? pluginIdOverride : memento
                .getString(TAG_PLUGIN_ID);
        return new ImageBindingDefinition(commandId, imageStyle, imageUri,
                locale, platform, pluginId);
    }

    static List readImageBindingDefinitions(IMemento memento, String name,
            String pluginIdOverride) {
        if (memento == null || name == null) throw new NullPointerException();
    ...
    static KeyConfigurationDefinition readKeyConfigurationDefinition(
        ...
        // TODO deprecated start
        if (parentId == null) parentId = memento.getString("parent"); //$NON-NLS-1$ 
        // TODO deprecated end
    ...
    static KeySequenceBindingDefinition readKeySequenceBindingDefinition(
        ...
        // TODO deprecated start
        if (contextId == null) contextId = memento.getString("scope"); //$NON-NLS-1$
        ...
        // TODO deprecated start
        if (commandId == null) commandId = memento.getString("command"); //$NON-NLS-1$ 

        if (commandId == null) commandId = memento.getString("id"); //$NON-NLS-1$ 
        // TODO deprecated end
        ...
        return new KeySequenceBindingDefinition(contextId, commandId,
                keyConfigurationId, keySequence, locale, platform, pluginId);
    }

    static List readKeySequenceBindingDefinitions(IMemento memento,
            String name, String pluginIdOverride) {
        if (memento == null || name == null) throw new NullPointerException();
        ...
        for (int i = 0; i < mementos.length; i++)
            list.add(readKeySequenceBindingDefinition(mementos[i],
                    pluginIdOverride));
    ...
    static void writeActiveKeyConfigurationDefinition(IMemento memento,
        ...
        memento.putString(TAG_PLUGIN_ID, activeKeyConfigurationDefinition
                .getPluginId());
    }
    ...
    static void writeContextBindingDefinition(IMemento memento,
            ContextBindingDefinition contextBindingDefinition) {
        if (memento == null || contextBindingDefinition == null)
                throw new NullPointerException();

        memento.putString(TAG_CONTEXT_ID, contextBindingDefinition
                .getContextId());
        memento.putString(TAG_COMMAND_ID, contextBindingDefinition
                .getCommandId());
        memento
                .putString(TAG_PLUGIN_ID, contextBindingDefinition
                        .getPluginId());
    }

    static void writeContextBindingDefinitions(IMemento memento, String name,
        ...
        while (iterator.hasNext())
            Util
                    .assertInstance(iterator.next(),
                            ContextBindingDefinition.class);

        iterator = contextBindingDefinitions.iterator();

        while (iterator.hasNext())
            writeContextBindingDefinition(memento.createChild(name),
    ...
    static void writeImageBindingDefinition(IMemento memento,
            ImageBindingDefinition imageBindingDefinition) {
        if (memento == null || imageBindingDefinition == null)
                throw new NullPointerException();

        memento
                .putString(TAG_COMMAND_ID, imageBindingDefinition
                        .getCommandId());
        memento.putString(TAG_IMAGE_STYLE, imageBindingDefinition
                .getImageStyle());
    ...
    static void writeImageBindingDefinitions(IMemento memento, String name,
        ...
        while (iterator.hasNext())
            writeImageBindingDefinition(memento.createChild(name),
    ...
    static void writeKeyConfigurationDefinition(IMemento memento,
        ...
        memento.putString(TAG_PARENT_ID, keyConfigurationDefinition
                .getParentId());
        memento.putString(TAG_PLUGIN_ID, keyConfigurationDefinition
                .getPluginId());
    }
    ...
    static void writeKeySequenceBindingDefinition(IMemento memento,
        ...
        memento.putString(TAG_CONTEXT_ID, keySequenceBindingDefinition
                .getContextId());
        memento.putString(TAG_COMMAND_ID, keySequenceBindingDefinition
                .getCommandId());
        memento.putString(TAG_KEY_CONFIGURATION_ID,
                keySequenceBindingDefinition.getKeyConfigurationId());
        memento.putString(TAG_KEY_SEQUENCE, keySequenceBindingDefinition
                .getKeySequence() != null ? keySequenceBindingDefinition
                .getKeySequence().toString() : null);
        memento.putString(TAG_LOCALE, keySequenceBindingDefinition.getLocale());
        memento.putString(TAG_PLATFORM, keySequenceBindingDefinition
                .getPlatform());
        memento.putString(TAG_PLUGIN_ID, keySequenceBindingDefinition
                .getPluginId());
    }

    static void writeKeySequenceBindingDefinitions(IMemento memento,
            String name, List keySequenceBindingDefinitions) {
        if (memento == null || name == null
        ...
        while (iterator.hasNext())
            writeKeySequenceBindingDefinition(memento.createChild(name),
