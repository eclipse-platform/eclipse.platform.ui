...
public class PerspectiveBarContributionItem extends ContributionItem {
	...
	public PerspectiveBarContributionItem(IPerspectiveDescriptor perspective, WorkbenchPage workbenchPage) {
		...
		menuManager.add(ContributionItemFactory.PERSPECTIVES_SHORTLIST.create(workbenchPage.getWorkbenchWindow()));
	...
	public void fill(ToolBar parent, int index) {
		if (toolItem == null && parent != null) {
			...
			if (imageDescriptor != null) {
				...
				toolItem.setImage(WorkbenchImages.getImageDescriptor(IWorkbenchGraphicConstants.IMG_CTOOL_DEF_PERSPECTIVE).createImage());
				toolItem.setHotImage(WorkbenchImages.getImageDescriptor(IWorkbenchGraphicConstants.IMG_CTOOL_DEF_PERSPECTIVE_HOVER).createImage());
			}

			toolItem.setToolTipText(WorkbenchMessages.format("PerspectiveBarContributionItem.toolTip", new Object[] { perspective.getLabel() })); //$NON-NLS-1$
	...
	public void update() {
		if (!toolItem.isDisposed()) {
			...
			if (preferenceStore.getBoolean(IWorkbenchPreferences.SHOW_TEXT_ON_PERSPECTIVE_BAR))
