...
public class EditorList {
	...
	private static final int NAME_SORT =
		IPreferenceConstants.EDITORLIST_NAME_SORT;
	private static final int MRU_SORT =
		IPreferenceConstants.EDITORLIST_MRU_SORT;

	private static final int SET_WINDOW_SCOPE =
		IPreferenceConstants.EDITORLIST_SET_WINDOW_SCOPE;
	private static final int SET_PAGE_SCOPE =
		IPreferenceConstants.EDITORLIST_SET_PAGE_SCOPE;
	private static final int SET_TAB_GROUP_SCOPE =
		IPreferenceConstants.EDITORLIST_SET_TAB_GROUP_SCOPE;
		...
		public void propertyChanged(Object source, int property) {
			if (property == IEditorPart.PROP_DIRTY
				...
				if (source instanceof IEditorPart) {
					EditorSite site =
						(EditorSite) ((IEditorPart) source).getEditorSite();
					IEditorReference ref =
						(IEditorReference) site.getPane().getPartReference();

					TableItem[] items = editorsTable.getItems();
					for (int i = 0; i < items.length; i++) {
						Adapter editor =
							(Adapter) items[i].getData(editorListData);
	...
	public Control createControl(Composite parent) {
		editorsTable =
			new Table(parent, SWT.MULTI | SWT.V_SCROLL | SWT.H_SCROLL);
		updateItems();
		editorsTable.pack();
	...
	private void notifyEditorListViews() {
		for (Iterator iterator = editorListViews.iterator();
			iterator.hasNext();
			) {
	...
	private void setCheckedMenuItems() {
		...
		if (dropDown) {
			tabGroupScopeAction.setChecked(
				EditorList.listScope == SET_TAB_GROUP_SCOPE);
		} else {
	...
	private void updateEditors(IWorkbenchPage[] pages) {
		for (int j = 0; j < pages.length; j++) {
			IEditorReference editors[] =
				((WorkbenchPage) pages[j]).getSortedEditors();
			...
			if (page != null) {
				if (pageScopeAction.isChecked()) {
					...
					for (int j = 0; j < editors.length; j++) {
						elements.add(
							new Adapter(editors[j].getEditorReference()));
					}
				}
			}
		}
	...
	private class SaveAction extends Action {
		...
		public void run() {
			...
			if (dirtyEditorList.size() != 0) {
				EditorManager.saveAll(dirtyEditorList, false, window, null);
			}
			destroyControl();
		}
	}
	...
	private class CloseAction extends Action {
		...
		public void run() {
			TableItem[] items = editorsTable.getSelection();
			final Adapter[] editorRef = new Adapter[items.length];
			List dirtyEditorList = new ArrayList();
			...
			IJobCompletionListener listener = new IJobCompletionListener() {
				/* (non-Javadoc)
				...
				public void aborted(IStatus status) {
						//Nothing to update if nothing happens
				...
				public void finished(IStatus status) {
					if (status instanceof StatusWithResult) {
						//now close the editors
						if (((StatusWithResult) status)
							.getResult()
							.equals(Boolean.TRUE)) {
			...
			if (dirtyEditorList.size() != 0) {
				EditorManager.saveAll(dirtyEditorList, true, window,listener);
			}
	...
	private class SelectionAction extends Action {
		...
		private TableItem[] invertSelection(
			TableItem[] allItems,
			TableItem[] selectedItems) {
			if (allItems.length == 0) {
				return allItems;
			}
			ArrayList invertedSelection =
				new ArrayList(allItems.length - selectedItems.length);
		...
		public void run() {
			switch (selectionType) {
				case SELECT_ALL :
					...
					break;
				case INVERT_SELECTION :
					editorsTable.setSelection(
						invertSelection(
							editorsTable.getItems(),
							editorsTable.getSelection()));
					break;
				case SELECT_CLEAN :
					editorsTable.setSelection(
						selectClean(editorsTable.getItems()));
					break;
			}
	...
	private class Adapter implements Comparable {
		...
		void close() {
			WorkbenchPage p =
				((WorkbenchPartReference) editorRef).getPane().getPage();
		...
		public int compareTo(Object another) {
			...
			int result =
				collator.compare(getText()[index], adapter.getText()[index]);
			return result;
		}
	}
}
