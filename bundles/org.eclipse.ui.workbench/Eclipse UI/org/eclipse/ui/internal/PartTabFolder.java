...
public class PartTabFolder extends LayoutPart implements ILayoutContainer {
	...
	/**
	 * Set the default bounds of a page in a CTabFolder2.
	 */
		...
		if (invisibleChildren != null) {
			...
			for (int i = 0, length = invisibleChildren.length;
				...
				if (invisibleChildren[i].part instanceof PartPlaceholder) {
					...
					TabInfo[] newStillInactive =
						new TabInfo[stillInactive.length + 1];
					System.arraycopy(
						stillInactive,
						0,
						newStillInactive,
					...
					newStillInactive[stillInactive.length] =
						invisibleChildren[i];
	...
	public void disableDrag(LayoutPart part) {
		PartDragDrop partDragDrop =
			(PartDragDrop) mapPartToDragMonitor.get(part);
	...
	public void dispose() {
		...
		if (invisibleChildren != null) {
			...
			newInvisibleChildren =
				new TabInfo[newInvisibleChildren.length
	...
	public void enableDrag(ViewPane pane, IPartDropListener listener) {
		...
		CTabPartDragDrop dragSource =
			new CTabPartDragDrop(pane, this.tabFolder, tab);
	...
	public void openTracker(LayoutPart part) {
		CTabPartDragDrop dnd =
			(CTabPartDragDrop) mapPartToDragMonitor.get(part);
		dnd.openTracker();
	}
	/**
	...
	public int getMinimumHeight() {
		...
		if (getItemCount() > 1) {
			Rectangle trim =
				tabFolder.computeTrim(0, 0, 0, current.getMinimumHeight());
			return trim.height;
		} else
	...
	/**
	 * Reorder the tab representing the specified pane. If a tab exists under
	 * the specified x,y location, then move the tab before it, otherwise place
	 * it as the last tab.
	 */
	...
	private void reorderTab(ViewPane pane, CTabItem2 sourceTab, int newIndex) {
		...
		CTabPartDragDrop partDragDrop =
			(CTabPartDragDrop) mapPartToDragMonitor.get(pane);
		...
		if (active) {
			...
			while (keys.hasNext()) {
				...
				if (part == oldChild) {
					...
					invisibleChildren =
						arrayAdd(invisibleChildren, info, index);
		...
		for (int i = 0, length = invisibleChildren.length; i < length; i++) {
			if (invisibleChildren[i].part == oldChild) {
				if (active) {
					...
					invisibleChildren =
						arrayRemove(invisibleChildren, oldChild);
					oldChild.setContainer(null);

					if (newChild instanceof PartPane) {
						WorkbenchPartReference ref =
							(WorkbenchPartReference) ((PartPane) newChild)
								.getPartReference();
					...
					invisibleChildren[i].part = newChild;
					// On restore, all views are initially represented by
					// placeholders and then
					// they are replaced with the real views. The following
					// code is used to preserve the active
					// tab when a prespective is restored from its persisted
					// state.
					if (inactiveCurrent != null
						&& inactiveCurrent == oldChild) {
						current = newChild;
	...
	public IStatus restoreState(IMemento memento) {
		// Read the active tab.
		String activeTabID =
			memento.getString(IWorkbenchConstants.TAG_ACTIVE_PAGE_ID);
		...
		if (children != null) {
			...
			for (int i = 0; i < children.length; i++) {
				...
				String partID =
					childMem.getString(IWorkbenchConstants.TAG_CONTENT);
				String tabText =
					childMem.getString(IWorkbenchConstants.TAG_LABEL);

				IViewDescriptor descriptor =
					(IViewDescriptor) WorkbenchPlugin
						.getDefault()
						.getViewRegistry()
						.find(
						partID);
	...
	public IStatus saveState(IMemento memento) {
		...
		if (current != null)
			memento.putString(
				IWorkbenchConstants.TAG_ACTIVE_PAGE_ID,
				current.getID());
			...
			if (invisibleChildren != null) {
				for (int i = 0; i < invisibleChildren.length; i++) {
					...
					childMem.putString(
						...
						info.tabText);
					childMem.putString(
						IWorkbenchConstants.TAG_CONTENT,
						info.part.getID());
				}
			}
		} else {
	...
	private void setSelection(LayoutPart part) {
		...
		if (window instanceof DetachedWindow) {
			...
			//$NON-NLS-1$
				...
				window.getShell().setText(
					((PartPane) current).getPartReference().getTitle());
		}
	}
	/**
