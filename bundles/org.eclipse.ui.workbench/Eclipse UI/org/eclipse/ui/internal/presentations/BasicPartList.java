	...
	private class BasicStackListLabelProvider extends LabelProvider implements
		...
		private int getMaxLength() {
			if (MAX_LENGTH < 0) {
				...
				// characters.
				MAX_LENGTH = Math.min(10, displayWidth
		...
		public Font getFont(Object element) {
			...
			// if in single tab mode, do not use the bold font for non-visible
			// tabs
		...
		public int compare(Viewer viewer, Object e1, Object e2) {
			...
			if (viewer == null || !(viewer instanceof ContentViewer)) {
				...
				if (prov instanceof ILabelProvider) {
					...
					// DefaultEditorPresentation.getLabelText which returns the
					// name of dirty
					// files begining with dirty prefix, sorting should not take
					// dirty prefix in consideration
	...
	protected TableViewer createTableViewer(Composite parent, int style) {
		...
		TableViewer tableViewer = new TableViewer(table) {
			...
			protected void internalRefresh(Object element) {
				...
				try {
					// This avoids a "graphic is disposed" error on Motif by not
					// letting
					// it redraw while we remove entries. Some items in this
					// table are
					// being removed and may have icons which may have already
					// been
					// disposed elsewhere.
					if (usingMotif) {
	...
	protected boolean deleteSelectedElements() {
		...
		if (structuredSelection != null) {
			...
			for (Iterator iterator = structuredSelection.iterator(); iterator
					.hasNext();) {
				IPresentablePart presentablePart = (IPresentablePart) iterator
						.next();
