...
public class DefaultPartPresentation extends StackPresentation {
    ...
    private final static String TAB_DATA = DefaultPartPresentation.class
            .getName()
            + ".partId"; //$NON-NLS-1$
    ...
    private MouseListener mouseListener = new MouseAdapter() {
        public void mouseDown(MouseEvent e) {
            if (e.widget instanceof Control) {
                ...
                // Else, set focus.
                CTabItem newItem = tabFolder.getItem(tabFolder.getControl()
                        .toControl(globalPos));
                if (newItem != null) {
        ...
        public void propertyChange(PropertyChangeEvent event) {
            ...
            if (property.equals(IThemeManager.CHANGE_CURRENT_THEME)) {
                ...
                setTitleAttributes();
                    ...
                    .equals(IWorkbenchThemeConstants.INACTIVE_TAB_BG_START)
                    || property
                            .equals(IWorkbenchThemeConstants.INACTIVE_TAB_BG_END)
                    || property
                            .equals(IWorkbenchThemeConstants.INACTIVE_TAB_TEXT_COLOR)
                    || property
                            .equals(IWorkbenchThemeConstants.ACTIVE_TAB_TEXT_COLOR)
                    || property
                            .equals(IWorkbenchThemeConstants.ACTIVE_TAB_BG_START)
                    || property
                            .equals(IWorkbenchThemeConstants.ACTIVE_TAB_BG_END)
                    || property.equals(IWorkbenchThemeConstants.TAB_TEXT_FONT)) {
                updateGradient();
            } else if (property
                    .equals(IWorkbenchThemeConstants.VIEW_MESSAGE_TEXT_FONT)) {
                setTitleAttributes();
            }
        }
    };

    public DefaultPartPresentation(PaneFolder control,
            IStackPresentationSite stackSite) {
        super(stackSite);

        shellActive = control.getControl().getShell().equals(
        ...
        tabFolder.setMinimizeVisible(stackSite
                .supportsState(IStackPresentationSite.STATE_MINIMIZED));
        tabFolder.setMaximizeVisible(stackSite
        ...
        dragListener = new Listener() {
            public void handleEvent(Event event) {
                ...
                if ((tabFolder.getControl().getStyle() & SWT.TOP) != 0) {
                    ...
                    if (localPos.y < tabFolder.getControl().getBounds().height
    ...
    protected void setTitleAttributes() {
        ...
        ITheme theme = PlatformUI.getWorkbench().getThemeManager()
                .getCurrentTheme();
        Font messageFont = theme.getFontRegistry().get(
    ...
    protected final void setActiveTabColors() {
        ...
        if (isShellActive()) {
            ...
            drawGradient(
                    colorRegistry
                            .get(IWorkbenchThemeConstants.ACTIVE_NOFOCUS_TAB_TEXT_COLOR),
                    new Color[] {
                            colorRegistry
                                    .get(IWorkbenchThemeConstants.ACTIVE_NOFOCUS_TAB_BG_START),
                            colorRegistry
                                    .get(IWorkbenchThemeConstants.ACTIVE_NOFOCUS_TAB_BG_END) },
                    new int[] { theme
                            .getInt(IWorkbenchThemeConstants.ACTIVE_NOFOCUS_TAB_PERCENT) },
                    theme
                            .getBoolean(IWorkbenchThemeConstants.ACTIVE_NOFOCUS_TAB_VERTICAL));
    ...
    protected void widgetDisposed() {
        ...
        PresentationUtil.removeDragListener(tabFolder.getControl(),
                dragListener);
            ...
            if (item.isShowing()) {
                ...
                if (lruListSize == 0)
                    ...
                    for (j = 0; j < lruListSize; j++) {
                        if (activationList.indexOf(getPartForTab(item)) < activationList
                                .indexOf(getPartForTab((CTabItem) lruList
                                        .get(j))))
                            break;
                    }
                    lruList.add(j, item);
    ...
    private int getItemInsertionIndex() {
        ...
        if (!tabFolder.getControl().isVisible()
                || tabFolder.getControl().getBounds().width <= 1)
            return idx;
        for (int i = 0; i < idx; i++) {
            CTabItem item = tabFolder.getItem(i);
    ...
    protected void showSystemMenu(Point point) {
        ...
        Menu aMenu = systemMenuManager.createContextMenu(tabFolder.getControl()
                .getParent());
    ...
    public StackDropResult dragOver(Control currentControl, Point location) {
        ...
        return new StackDropResult(Geometry.toDisplay(tabFolder.getControl(),
                tabUnderPointer.getBounds()), new Integer(tabFolder
    ...
    public void showSystemMenu() {
        ...
        IPresentablePart part = getCurrentPart();
        if (part != null) {
            Rectangle bounds = DragUtil
                    .getDisplayBounds(tabFolder.getControl());
    ...
    public Control[] getTabList(IPresentablePart part) {
        ...
        if (tabFolder.getTabPosition() == SWT.BOTTOM) {
            ...
            if (getTabFolder() != null)
                list.add(getTabFolder().getControl());
        } else {
            if (getTabFolder() != null)
                list.add(getTabFolder().getControl());
            if (part.getToolBar() != null)
    ...
    protected void showList(Shell parentShell, int x, int y) {
        ...
        final BasicStackList editorList = new BasicStackList(tabFolder
                .getControl().getShell(), shellStyle, tableStyle);
    ...
    public void showPartList() {
        ...
        Point location = item.getDisplay().map(tabFolder.getControl(), null, x,
                y);
