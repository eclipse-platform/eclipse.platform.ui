...
public class CTabItem extends Item {
	...
	void drawClose(GC gc) {
		...
		case CTabFolder.NORMAL: {
			int[] shape = new int[] { x, y, x + 2, y, x + 4, y + 2, x + 5,
		...
		case CTabFolder.SELECTED: {
			int[] shape = new int[] { x + 1, y + 1, x + 3, y + 1, x + 5, y + 3,
					x + 6, y + 3, x + 8, y + 1, x + 10, y + 1, x + 10, y + 3,
					x + 8, y + 5, x + 8, y + 6, x + 10, y + 8, x + 10, y + 10,
					x + 8, y + 10, x + 6, y + 8, x + 5, y + 8, x + 3, y + 10,
					x + 1, y + 10, x + 1, y + 8, x + 3, y + 6, x + 3, y + 5,
					x + 1, y + 3 };
			Color fill = new Color(display, CTabFolder.CLOSE_FILL);
			gc.setBackground(fill);
			gc.fillPolygon(shape);
		...
		case CTabFolder.NONE: {
			int[] shape = new int[] { x, y, x + 10, y, x + 10, y + 10, x,
					y + 10 };
	...
	void drawSelected(GC gc) {
		...
		int yy = parent.onBottom ? size.y - parent.borderBottom
				- parent.tabHeight - parent.highlight_header : parent.borderTop
				+ parent.tabHeight + 1;
		int ww = size.x - parent.borderLeft - parent.borderRight;
		int hh = parent.highlight_header - 1;
		int[] shape = new int[] { xx, yy, xx + ww, yy, xx + ww, yy + hh, xx,
				yy + hh };
		...
		if (parent.single) {
			...
			if (this.parent.onBottom) {
				...
				for (int i = 0; i < right.length / 2; i++) {
					shape[index++] = parent.simple ? rightEdge - 1
							+ right[2 * i] : rightEdge - parent.curveIndent
							+ right[2 * i];
					shape[index++] = parent.simple ? y + height
							+ right[2 * i + 1] - 1 : y + right[2 * i + 1] - 2;
				}
			...
			parent.antialias(shape, CTabFolder.borderColor.getRGB(), inside,
					outside, gc);
			gc.setForeground(CTabFolder.borderColor);
			...
			if (parent.isFocusControl()) {
				...
				if (parent.simple || parent.single) {
					...
					gc.setForeground(display
							.getSystemColor(CTabFolder.BUTTON_BORDER));
	...
	public Rectangle getBounds() {
		...
		int w = width;
		if (!parent.simple && !parent.single
				&& parent.indexOf(this) == parent.selectedIndex)
				...
				if (bounds.width == oldBounds.width
					...
					if (showing) {
						...
						if (selected || parent.showUnselectedImage) {
							...
							if (selected) {
								if (parent.single
										&& (parent.showClose || showClose))
									imageX += CTabFolder.BUTTON_SIZE;
								int rightEdge = Math.min(x + width, parent
										.getRightItemEdge());
								maxImageWidth = rightEdge - imageX
										- RIGHT_MARGIN;
								if (!parent.single && closeRect.width > 0)
									maxImageWidth -= closeRect.width
											+ INTERNAL_SPACING;
							} else {
								maxImageWidth = x + width - imageX
										- RIGHT_MARGIN;
							...
							if (bounds.width < maxImageWidth) {
								int imageY = y + (height - bounds.height) / 2
										+ (parent.onBottom ? -1 : 1);
								parent.redraw(imageX, imageY, bounds.width,
										bounds.height, false);
							}
						}
					}
