 ...
 * type <code>CTabItem</code>. <code>Control</code> children are created
 * and then set into a tab item using <code>CTabItem#setControl</code>.
 * </p>
 * <p>
...
public class CTabFolder extends Composite {
	 ...
	 * compatability. It should not be capitalized.
	 * 
	 ...
	 * 
	 * NOTE This field is badly named and can not be fixed for backwards
	 * compatability. It should be capitalized.
	 * 
	...
	// @see StackPresentation#AS_ACTIVE_FOCUS
	// @see StackPresentation#AS_ACTIVE_NOFOCUS
	// @see StackPresentation#AS_INACTIVE
	int activationState = StackPresentation.AS_INACTIVE;
	...
	public CTabFolder(Composite parent, int style) {
		...
		int[] folderEvents = new int[] { SWT.Dispose, SWT.DragDetect,
				SWT.FocusIn, SWT.FocusOut, SWT.KeyDown, SWT.MouseDoubleClick,
				SWT.MouseDown, SWT.MouseEnter, SWT.MouseExit, SWT.MouseMove,
				SWT.MouseUp, SWT.Paint, SWT.Resize, SWT.Traverse, };
	...
	static int checkStyle(Composite parent, int style) {
		int mask = SWT.CLOSE | SWT.TOP | SWT.BOTTOM | SWT.FLAT
				| SWT.LEFT_TO_RIGHT | SWT.RIGHT_TO_LEFT | SWT.SINGLE
				| SWT.MULTI;
		style = style & mask;
		...
		if (outerRGB != null) {
			...
			for (int i = 0; i < shape.length / 2; i++) {
				if (left && (index + 3 < shape.length)) {
					left = onBottom ? oldY <= shape[index + 3]
							: oldY >= shape[index + 3];
					oldY = shape[index + 1];
				}
	...
	public Rectangle computeTrim(int x, int y, int width, int height) {
		...
		int trimWidth = width + borderLeft + borderRight + 2 * marginWidth + 2
				* highlight_margin;
		if (minimized) {
			int trimY = onBottom ? y - borderTop : y - highlight_header
					- tabHeight - borderTop;
			int trimHeight = borderTop + borderBottom + tabHeight
					+ highlight_header;
			return new Rectangle(trimX, trimY, trimWidth, trimHeight);
		} else {
			int trimY = onBottom ? y - marginHeight - highlight_margin
					- borderTop : y - marginHeight - highlight_header
					- tabHeight - borderTop;
			int trimHeight = height + borderTop + borderBottom + 2
					* marginHeight + tabHeight + highlight_header
					+ highlight_margin;
			return new Rectangle(trimX, trimY, trimWidth, trimHeight);
		}
	}
	...
	void createItem(CTabItem item, int index) {
		...
		for (int i = 0; i < priority.length; i++) {
			...
			newPriority[next++] = priority[i] >= index ? priority[i] + 1
					: priority[i];
		}
	...
	void drawBackground(GC gc, int[] shape, boolean selected) {
		...
		int[] percents = selected ? selectionGradientPercents
				: gradientPercents;
		boolean vertical = selected ? selectionGradientVertical
				: gradientVertical;
		Point size = getSize();
		int width = size.x;
	...
	void drawBackground(GC gc, int[] shape, int x, int y, int width,
		...
		if (image != null) {
			...
			// draw gradient
			if (colors.length == 1) {
				Color background = colors[0] != null ? colors[0]
						: defaultBackground;
			...
			if ((getStyle() & SWT.NO_BACKGROUND) != 0
					|| !defaultBackground.equals(getBackground())) {
				gc.setBackground(defaultBackground);
				gc.fillRectangle(x, y, width, height);
			}
		}
	...
	void drawBody(Event event) {
		...
		// fill in body
		if (!minimized) {
			int width = size.x - borderLeft - borderRight - 2
					* highlight_margin;
		...
		if (borderLeft > 0) {
			...
			int y2 = onBottom ? size.y - tabHeight - borderBottom - 1 : size.y
					- borderBottom;
	...
	void drawChevron(GC gc) {
		...
		if (single) {
			...
			int showCount = 0;
			while (showCount < priority.length
					&& items[priority[showCount]].showing) {
				showCount++;
			}
		...
		case NORMAL: {
			Color chevronBorder = single ? getSelectionForeground()
					: getForeground();
	...
	void drawTabArea(Event event) {
		...
		// Draw Tab Header
		if (onBottom) {
			int[] left = simple ? SIMPLE_BOTTOM_LEFT_CORNER
					: BOTTOM_LEFT_CORNER;
			int[] right = simple ? SIMPLE_BOTTOM_RIGHT_CORNER
					: BOTTOM_RIGHT_CORNER;
		...
		if (selectedIndex != -1) {
			...
			int x1 = borderLeft;
			int y1 = (onBottom) ? size.y - borderBottom - tabHeight - 1
					: borderTop + tabHeight;
			int x2 = size.x - borderRight;
			gc.setForeground(borderColor);
	...
	public Rectangle getClientArea() {
		...
		Point size = getSize();
		int width = size.x - borderLeft - borderRight - 2 * marginWidth - 2
				* highlight_margin;
	...
	/**
	 * Returns <code>true</code> if the CTabFolder only displys the selected
	 * tab and <code>false</code> if the CTabFolder displays multiple tabs.
	 * 
	 * @return <code>true</code> if the CTabFolder only displys the selected
	 ...
	 * 
	 * @return <code>true</code> if the close button appears on unselected
	 *         tabs
	 * 
	 * @since 3.0
	 */
		...
		accessible.addAccessibleControlListener(new AccessibleControlAdapter() {
			...
			public void getDefaultAction(AccessibleControlEvent e) {
				...
				if (childID >= items.length
						&& childID < items.length + EXTRA_CHILD_ID_COUNT) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (i != selectedIndex && item.closeImageState != NONE) {
					...
					redraw(item.closeRect.x, item.closeRect.y,
							item.closeRect.width, item.closeRect.height, false);
				}
		...
		case SWT.MouseMove: {
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (item.getBounds().contains(x, y)) {
					close = true;
					if (item.closeRect.contains(x, y)) {
						if (item.closeImageState != SELECTED
					...
					if (e.doit) {
						...
						if (nextItem != null) {
							if (nextItem.closeRect.contains(pt)) {
								if (nextItem.closeImageState != SELECTED
	 ...
	 * 
	 * <pre>
	 * cfolder.setBackground(new Color[] {
	...
	void setButtonBounds() {
		...
		if (oldX != minRect.x || oldWidth != minRect.width || oldY != minRect.y
				|| oldHeight != minRect.height) {
		...
		if (topRight != null) {
			...
			case SWT.FILL: {
				int rightEdge = size.x - borderRight - 3 - maxRect.width
						- minRect.width;
				...
				if (single) {
					if (items.length == 0 || selectedIndex == -1) {
						...
						if (item.x + item.width + 7 + 3 * BUTTON_SIZE / 2 >= rightEdge)
							break;
						topRightRect.x = item.x + item.width + 7 + 3
								* BUTTON_SIZE / 2;
						topRightRect.width = rightEdge - topRightRect.x;
					}
				} else {
					...
					topRightRect.width = Math
							.max(0, rightEdge - topRightRect.x);
				}
			...
			case SWT.RIGHT: {
				Point topRightSize = topRight.computeSize(SWT.DEFAULT,
						tabHeight, false);
				int rightEdge = size.x - borderRight - 3 - maxRect.width
						- minRect.width;
				if (!simple && borderRight > 0 && !showMax && !showMin)
		...
		if (oldX != topRightRect.x || oldWidth != topRightRect.width
			...
			int right = Math.max(oldX + oldWidth, topRightRect.x
					+ topRightRect.width);
			int top = onBottom ? size.y - borderBottom - tabHeight
					: borderTop + 1;
			redraw(left, top, right - left, tabHeight, false);
		...
		if (single) {
			if (selectedIndex == -1 || items.length > 1) {
				...
				chevronRect.y = onBottom ? size.y - borderBottom - tabHeight
						+ (tabHeight - chevronRect.height) / 2 : borderTop
						+ (tabHeight - chevronRect.height) / 2;
				if (selectedIndex == -1) {
					chevronRect.x = size.x - borderRight - 3 - minRect.width
							- maxRect.width - topRightRect.width
							- chevronRect.width;
				} else {
					CTabItem item = items[selectedIndex];
					int w = size.x - borderRight - 3 - minRect.width
							- maxRect.width - chevronRect.width;
		...
		if (oldX != chevronRect.x || oldWidth != chevronRect.width
			...
			int right = Math.max(oldX + oldWidth, chevronRect.x
					+ chevronRect.width);
			int top = onBottom ? size.y - borderBottom - tabHeight
					: borderTop + 1;
			redraw(left, top, right - left, tabHeight, false);
	...
	boolean setItemLocation() {
		...
		if (items.length == 0)
			return false;
		Point size = getSize();
		int y = onBottom ? Math.max(borderBottom, size.y - borderBottom
				- tabHeight) : borderTop;
		if (single) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (i == selectedIndex) {
					...
					if (showClose || item.showClose) {
						...
						item.closeRect.y = onBottom ? size.y - borderBottom
								- tabHeight + (tabHeight - BUTTON_SIZE) / 2
			...
			for (int i = 0; i < priority.length; i++) {
				CTabItem item = items[priority[i]];
				width += item.width;
				item.showing = i == 0 ? true : item.width > 0
						&& width <= maxWidth;
			...
			for (int i = 0; i < items.length; i++) {
				...
				if (!item.showing) {
					...
					if (i == selectedIndex) {
						...
						item.closeRect.x = edge - CTabItem.RIGHT_MARGIN
								- BUTTON_SIZE;
					} else {
						item.closeRect.x = item.x + item.width
								- CTabItem.RIGHT_MARGIN - BUTTON_SIZE;
					}
					item.closeRect.y = onBottom ? size.y - borderBottom
							- tabHeight + (tabHeight - BUTTON_SIZE) / 2
							: borderTop + (tabHeight - BUTTON_SIZE) / 2;
	...
	boolean setItemSize() {
		...
		if (minWidth > tabAreaWidth) {
			...
			for (int i = 0; i < items.length; i++) {
				maxWidths[i] = items[i].preferredWidth(gc, i == selectedIndex,
						false);
	...
	/**
	 * Set the selection background and indicate which selection state this
	 * represents.
	 * 
	 * @param colors
	 * @param percents
	 * @param vertical
	 * @param activationState. Value of -1 is ignored
	 * @see StackPresentation#AS_ACTIVE_FOCUS
	 * @see StackPresentation#AS_ACTIVE_NOFOCUS
	 * @see StackPresentation#AS_INACTIVE
	 */
	public void setSelectionBackground(Color[] colors, int[] percents,
		...
		setSelectionBackground(colors, percents, vertical);
	 ...
	 * 
	 * @param simple
	 *            <code>true</code> if the CTabFolder should render itself in
	 *            a simple, traditional style
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 * 
	 * @param single
	 *            <code>true</code> if only the selected tab should be
	 *            displayed otherwise, multiple tabs will be shown.
	 * 
	 * @exception SWTException
	 *                <ul>
	 *                <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 ...
	 * <code>SWT.RIGHT</code> will cause the control to be positioned on the
	 * far right of the folder and it will have its default size.
	...
	boolean updateTabHeight(boolean force) {
		...
		if (fixedTabHeight != SWT.DEFAULT) {
			tabHeight = fixedTabHeight == 0 ? 0 : fixedTabHeight + 1; // +1
																		// for
																		// line
																		// drawn
																		// across
			...
			if (items.length == 0) {
				...
				for (int i = 0; i < items.length; i++) {
					tempHeight = Math.max(tempHeight, items[i]
							.preferredHeight(gc));
				}
			}
			gc.dispose();
	...
	String _getToolTip(int x, int y) {
		if (showMin && minRect.contains(x, y))
			return minimized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Minimize"); //$NON-NLS-1$ //$NON-NLS-2$
		if (showMax && maxRect.contains(x, y))
			return maximized ? SWT.getMessage("SWT_Restore") : SWT.getMessage("SWT_Maximize"); //$NON-NLS-1$ //$NON-NLS-2$
