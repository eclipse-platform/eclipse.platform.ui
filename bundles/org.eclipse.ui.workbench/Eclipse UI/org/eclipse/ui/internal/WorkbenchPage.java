...
public class WorkbenchPage implements IWorkbenchPage {
	...
	public boolean closeAllEditors(boolean save) {
		...
		// Save part.
		if (save) {
			BooleanJobCompletionListener listener =
				new BooleanJobCompletionListener();
			getEditorManager().saveAll(true, true, listener);
	...
	public boolean closeEditor(IEditorPart editor, boolean save) {
		...
		if (save) {
			BooleanJobCompletionListener listener =
				new BooleanJobCompletionListener();
			getEditorManager().saveEditor(editor, true, listener);
			if (!listener.completed())
				return false;
		}
	...
	public boolean editActionSets() {
		...
		ActionSetSelectionDialog dlg =
			new ActionSetSelectionDialog(window.getShell(), persp);
	...
	private IEditorPart openMarker(
		IMarker marker,
		boolean activate,
	...
	WorkbenchMessages.format("WorkbenchPage.editorAlreadyOpenedMsg", new String[] { input.getName()}), //$NON-NLS-1$
		...
		if (useEditorID)
			...
			ref =
				getEditorManager().openEditor(null, input, true, forceInternal);
	...
	public boolean saveAllEditors(boolean confirm) {
		BooleanJobCompletionListener listener =
			new BooleanJobCompletionListener();
		getEditorManager().saveAll(confirm, false, listener);
	...
	protected boolean savePart(
		...
		BooleanJobCompletionListener listener =
			new BooleanJobCompletionListener();
		getEditorManager().savePart(saveable, part, confirm, listener);
		return listener.completed();
