...
public class IntroRegistry implements IIntroRegistry {
	...
	public void addBinding(IConfigurationElement element) throws CoreException {
	    ...
	    if (introId == null || productId == null) {
	        IStatus status = new Status(IStatus.ERROR, element
					.getDeclaringExtension().getNamespace(), IStatus.ERROR, "introId and productId must be defined.", new IllegalArgumentException()); //$NON-NLS-1$
	        throw new CoreException(status);
	    }
	    if (bindingMap.containsKey(productId)) {
	        IStatus status = new Status(IStatus.WARNING, element
					.getDeclaringExtension().getNamespace(), IStatus.WARNING, productId + " already has an intro binding.  Omitting binding to" + introId, new IllegalArgumentException()); //$NON-NLS-1$
	        throw new CoreException(status);			
	    }
