...
import org.eclipse.core.internal.plugins.PreferenceExporter;
...
/**
 * The wizard responsible for handling both the import and export of preferences
 * to/from files.  There is a lot of overlap in functionality, so these wizards
 * are simply implemented as one wizard with two modes.
 * 
 * @since 3.0
 */
public class PreferenceImportExportWizard extends Wizard {
	 ...
	 * second page shown to the user.  This valud should not be 
	 * <code>null</code> after the pages have been added.
	 */
	 ...
	 * @param exportWizard Whether the wizard should act as an export tool.
	 * @param parentDialog The dialog which created this wizard (<em>not</em>
	 * the wizard dialog itself).  This parameter should not be 
	 * <code>null</code>.
	 */
	public PreferenceImportExportWizard(final boolean exportWizard, FilteredPreferenceDialog parentDialog) {
		super();
	...
	public boolean performFinish() {		
		...
		Iterator nodes = parent.getPreferenceManager().getElements(PreferenceManager.PRE_ORDER).iterator();
		while (nodes.hasNext()) {
		...
		try {
			...
			if (export) {
				...
				PreferenceExporter.importPreferences(path);
			}
			Preferences.exportPreferences(new Path(selectedFilePath));
			
		} catch (CoreException e) {
			ErrorDialog.openError(
				getShell(), 
				WorkbenchMessages.getString("WorkbenchPreferenceDialog.saveErrorTitle"), //$NON-NLS-1$
				WorkbenchMessages.format("WorkbenchPreferenceDialog.saveErrorMessage", new Object[]{selectedFilePath}), //$NON-NLS-1$
				e.getStatus());
				return false;
		...
		if (!export) {
			MessageDialog.openInformation(
				...
				WorkbenchMessages.format("WorkbenchPreferenceDialog.loadMessage", new Object[]{selectedFilePath})); //$NON-NLS-1$
			...
			MessageDialog.openInformation(
				...
				WorkbenchMessages.format("WorkbenchPreferenceDialog.saveMessage", new Object[]{selectedFilePath})); //$NON-NLS-1$
		...
		WorkbenchPlugin.getDefault().getDialogSettings().put(WorkbenchPreferenceDialog.FILE_PATH_SETTING, fileSelectionPage.getPath());
		return true;
	}
}
