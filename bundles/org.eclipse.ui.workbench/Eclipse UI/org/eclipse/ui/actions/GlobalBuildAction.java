 ...
 * </p>
 */
public class GlobalBuildAction extends BackgroundAction {
	...
	void doBuildOperation() {
			...
			final MultiStatus status = new MultiStatus(PlatformUI.PLUGIN_ID, 0, WorkbenchMessages.getString("GlobalBuildAction.buildProblems"), //$NON-NLS-1$
	null);
		...
		try {
			...
			reportException(
				e,
				WorkbenchMessages.getString("GlobalBuildAction.buildProblems"));
			return;
		}
		...
		if (!status.isOK()) {
			...
			shell.getDisplay().asyncExec(new Runnable() {
				 ...
				 */
				public void run() {
					ErrorDialog.openError(shell, WorkbenchMessages.getString("GlobalBuildAction.problemTitle"), //$NON-NLS-1$
