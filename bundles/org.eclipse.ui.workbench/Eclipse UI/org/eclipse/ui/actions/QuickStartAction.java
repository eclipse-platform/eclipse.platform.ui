...
public class QuickStartAction extends Action {
	...
	/**
	 * The user has invoked this action. Prompts for a feature with a welcome
	 * page, then opens the corresponding welcome page.
	 */
	public void run() {
	...
	private AboutInfo promptForFeature() {
		...
		if (welcomeFeatures.size() == 0) {
			MessageDialog.openInformation(shell, WorkbenchMessages.getString("QuickStartMessageDialog.title"), //$NON-NLS-1$
		...
		AboutInfo primaryFeature = ((Workbench) window.getWorkbench()).getConfigurationInfo().getAboutInfo();
			...
			FeatureSelectionDialog d = new FeatureSelectionDialog(shell, features, primaryFeature, "WelcomePageSelectionDialog.title", //$NON-NLS-1$
	...
	private boolean openWelcomePage(AboutInfo feature) {
		...
		try {
			...
			ErrorDialog.openError(window.getShell(), WorkbenchMessages.getString("Workbench.openEditorErrorDialogTitle"), //$NON-NLS-1$
