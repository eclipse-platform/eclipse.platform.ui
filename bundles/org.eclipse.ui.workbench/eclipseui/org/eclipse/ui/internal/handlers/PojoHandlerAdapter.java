...
class PojoHandlerAdapter extends AbstractHandler {
	...
	/**
	 * Resolves the context to run the handler in: the active leaf below the
	 * {@link IEclipseContext} the given evaluation object was created from, as the
	 * E4 handler service does, since legacy enablement updates pass the window
	 * state rather than the active part.
	 */
	private IEclipseContext executionContextOf(Object evaluationObject) {
		IEclipseContext context = null;
		if (evaluationObject instanceof IEclipseContext eclipseContext) {
			context = eclipseContext;
		} else if (evaluationObject instanceof IEvaluationContext evaluationContext
				&& evaluationContext.getVariable(IEclipseContext.class.getName()) instanceof IEclipseContext eclipseContext) {
			context = eclipseContext;
		} else {
