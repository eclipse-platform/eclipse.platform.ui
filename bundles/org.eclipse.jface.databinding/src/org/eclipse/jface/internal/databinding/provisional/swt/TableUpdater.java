...
public abstract class TableUpdater {
	...
	private class UpdateRunnable implements Runnable, IChangeListener,
		...
		public void run() {
			if (theTable != null && !theTable.isDisposed()
					&& theTable.isVisible() && item != null
					&& !item.isDisposed()) {
				updateIfNecessary();
			}
		}
	...
	public TableUpdater(Table toUpdate) {
		...
		theTable.addListener(SWT.MeasureItem, privateInterface);
	}
	...
	public void dispose() {
		...
		theTable.removeListener(SWT.MeasureItem, privateInterface);
