...
abstract class AbstractColumnLayout extends Layout {
	...
	/**
	 * Comput the size of the table or tree based on the ColumnLayoutData and the 
	 * width and height hint.
	...
	private void layoutTableTree(final Scrollable scrollable, final int width, final Rectangle area, final boolean increase) {
		final int columnCount = getColumnCount(scrollable);
		final int size= Math.min(columns.size(), columnCount);
	...
	private int computeTrim(Rectangle area, Scrollable scrollable, int currentWidth) {
		Point preferredSize= computeTableTreeSize(scrollable, area.width, area.height);
        int trim;
