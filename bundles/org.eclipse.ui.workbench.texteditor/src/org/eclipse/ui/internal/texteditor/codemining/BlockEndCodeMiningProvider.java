 ...
 * partitioner for the {@link IDocumentExtension3#DEFAULT_PARTITIONING default
 * partitioning}, braces inside non-default partitions (typically comments and
...
public class BlockEndCodeMiningProvider extends AbstractCodeMiningProvider implements IPropertyChangeListener {
	...
	private static ITypedRegion[] computeCodeRegions(IDocument document) throws BadLocationException {
		ITypedRegion[] partitions= TextUtilities.computePartitioning(document, IDocumentExtension3.DEFAULT_PARTITIONING, 0,
				document.getLength(), false);
		if (partitions.length > 0) {
			return partitions;
		}
		// No partitioner installed for the default partitioning; treat the whole
		// document as code.
