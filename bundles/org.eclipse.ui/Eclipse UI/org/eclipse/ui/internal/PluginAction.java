...
public class PluginAction
	...
	private boolean hasAdaptableType() {
		if (adaptableNotChecked) {
			String typeName = configElement.getAttribute("objectClass");
	...
	public void selectionChanged(IStructuredSelection selection) {
		...
		if (hasAdaptableType())
			this.selection = getResourceAdapters(selection);
	...
	private IStructuredSelection getResourceAdapters(IStructuredSelection selection) {
		...
		IAdaptable[] elements = (IAdaptable[]) selection.toArray();
		for (int i = 0; i < elements.length; i++) {
			Object adaptedValue = elements[i].getAdapter(IResource.class);
