...
public class WorkingSetDialog implements IWorkingSetDialog {
	...
	private class WorkingSetDialogWidget extends InputDialog {
		...
		WorkingSetDialogWidget(Shell parentShell) {
		...
		private WorkingSetDialogWidget(Shell parentShell, String initialSelection) {
			super(
				parentShell,
				WorkbenchMessages.getString("WorkingSetDialog.title"),		//$NON-NLS-1$
	...
	private static class WorkingSetNameInputValidator implements IInputValidator {
		...
		public String isValid(String newText) {
			...
			if (!newText.equals(initalName)) {
				IWorkingSet[] workingSets = WorkbenchPlugin.getDefault().getWorkingSetRegistry().getWorkingSets();
	...
	public IWorkingSet getWorkingSet() {
		...
		return dialogWidget.getWorkingSet();
	}
	public void init(Shell shell) {
		dialogWidget = new WorkingSetDialogWidget(shell);
	}
	public void init(Shell shell, IWorkingSet workingSet) {
		dialogWidget = new WorkingSetDialogWidget(shell, workingSet);
	}
	public int open() {
		if (dialogWidget == null) {
			return CANCEL;
