...
import org.eclipse.toolscript.core.internal.ToolScriptBuilder;
 ...
 */
public final class ToolScriptBuilderPropertyPage extends PropertyPage {
	 ...
	 */
	public ToolScriptBuilderPropertyPage() {
		super();
	 ...
	 */
	private ICommand createToolScript() {
		try {
	...
	/**
	 * Edits an exisiting build command that invokes a tool script.
	 */
	private void editToolScript(ICommand command) {
		ToolScript script = ToolScript.fromArgumentMap(command.getArguments());
		if (script == null)
			return;
		ToolScriptEditDialog dialog;
		dialog = new ToolScriptEditDialog(getShell(), script);
		dialog.open();
		script = dialog.getToolScript();
	...
	protected Control createContents(Composite parent) {
		antImage = ToolScriptPlugin.getDefault().getImageDescriptor(ToolScriptPlugin.IMG_ANT_SCRIPT).createImage();
		builderImage = ToolScriptPlugin.getDefault().getImageDescriptor(ToolScriptPlugin.IMG_BUILDER).createImage();
		...
		description.setText(ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.description")); //$NON-NLS-1$
		...
		newButton = createButton(buttonArea, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.newButton")); //$NON-NLS-1$
		editButton = createButton(buttonArea, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.editButton")); //$NON-NLS-1$
		removeButton = createButton(buttonArea, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.removeButton")); //$NON-NLS-1$
		Label buttonSpacer = new Label(buttonArea, SWT.LEFT);
		upButton = createButton(buttonArea, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.upButton")); //$NON-NLS-1$
		downButton = createButton(buttonArea, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.downButton")); //$NON-NLS-1$
	...
	private void handleButtonPressed(Button button) {
		if (button == newButton) {
			...
			if (selection != null) {
				editToolScript((ICommand)selection[0].getData());
	...
	private void handleException(Exception e) {
		...
		if (e instanceof CoreException) {
			...
			status = new Status(IStatus.ERROR, ToolScriptPlugin.PLUGIN_ID, 0, ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.statusMessage"), e); //$NON-NLS-1$
		}
		ErrorDialog.openError(
			getShell(),
			ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.errorTitle"), //$NON-NLS-1$
			ToolScriptMessages.getString("ToolScriptBuilderPropertyPage.errorMessage"), //$NON-NLS-1$
			status);
	}
	...
	private void handleTableSelectionChanged() {
		...
		if (items != null && items.length == 1) {
			...
			if (buildCommand.getBuilderName().equals(ToolScriptBuilder.ID)) {
	...
	private void updateCommandItem(TableItem item, ICommand command) {
		...
		if (builderName.equals(ToolScriptBuilder.ID)) {
			ToolScript script = ToolScript.fromArgumentMap(command.getArguments());
			item.setText(script.getName());
			if (script.SCRIPT_TYPE_ANT.equals(script.getType())) {
