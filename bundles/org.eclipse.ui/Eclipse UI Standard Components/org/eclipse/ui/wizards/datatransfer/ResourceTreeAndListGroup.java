...
/**
 * Add all of the children of nextEntry to result recursively if it is not expanded. 
 * This does not set any values in the checked state.
 * @param nextEntry the treeElement being queried
 * @param result - the Collection the new children should be added to
*/
private void findAllUnExpandedChildren(Object treeElement, Collection result) {
	...
	for (int i = 0; i < treeChildren.length; i++) {
		findAllUnExpandedChildren(treeChildren[i], result);
	}

}
/**
 *	Answer a flat collection of all of the checked elements in the
 *	list portion of self. Also expand any white selected children
 * and add them.
 *
...
public Iterator getAllCheckedListItems() {
	...
	//Preprocess to add all of the non expanded white selected items to the checked list
	Iterator whiteCheckedEntries = whiteCheckedTreeItems.iterator();
	while (whiteCheckedEntries.hasNext()) {
		Object nextEntry = whiteCheckedEntries.next();
		if (!expandedTreeNodes.contains(nextEntry)) {
			//Expand it now
			findAllUnExpandedChildren(nextEntry, result);
		}
	}

	//Now add the expanded ones to the list
